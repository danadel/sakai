/**********************************************************************************
 * $URL: https://source.sakaiproject.org/svn/msub/upv.es/PoliformaT/upv-2.9.3/web/web-portlet/src/java/org/sakaiproject/portlets/PortletIFrame.java $
 * $Id: PortletIFrame.java 307467 2014-03-26 10:51:11Z anueda@asic.upv.es $
 ***********************************************************************************
 *
	...
	public void processActionEdit(ActionRequest request, ActionResponse response)
		throws PortletException, IOException 
		{
            ...
            if (SPECIAL_WORKSITE.equals(special))
            {
    			...
    			FormattedText.processFormattedText(description, error, true, false, true);
    			String[] error_linea = error.toString().split(String.valueOf('\n')); 
    			
    			if (error.toString().contains("iframe") && error_linea.length <=2) {
    				description = FormattedText.processFormattedText(description, error, false, false, true);
    			}
    			else {
    				description = FormattedText.processFormattedText(description, error, true, false, true);
    			}
