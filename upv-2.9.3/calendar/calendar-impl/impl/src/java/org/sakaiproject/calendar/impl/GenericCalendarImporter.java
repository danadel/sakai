/**********************************************************************************
 * $URL: https://source.sakaiproject.org/svn/msub/upv.es/PoliformaT/upv-2.9.3/calendar/calendar-impl/impl/src/java/org/sakaiproject/calendar/impl/GenericCalendarImporter.java $
 * $Id: GenericCalendarImporter.java 132551 2013-12-13 12:19:06Z vacercon@upvnet.upv.es $
 ***********************************************************************************
 *
...
public class GenericCalendarImporter implements CalendarImporterService {
	...
	private DateFormat timeFormatter() {
		DateFormat rv = new SimpleDateFormat(rb.getString("import.time.format"));
		rv.setLenient(false);
		return rv;
	}

	private DateFormat timeFormatterWithSeconds() {
		return new SimpleDateFormat(rb.getString("import.time.format.seconds"));
	}

	private DateFormat time24HourFormatter() {
		DateFormat rv = new SimpleDateFormat(rb.getString("import.time.format.24h"));
		rv.setLenient(false);
		return rv;
	}

	private DateFormat time24HourFormatterWithSeconds() {
		DateFormat rv = new SimpleDateFormat(rb.getString("import.time.format.24h.seconds"));
		rv.setLenient(false);
		return rv;
	}
	...
	public class PrototypeEvent implements CalendarEventEdit {
		 ...
		 * (non-Javadoc)
		 * 
		 * @see org.sakaiproject.service.legacy.entity.AttachmentContainerEdit#
	...
	public List doImport(String importType, InputStream importStream,
		...
		scheduleImport.importStreamFromDelimitedFile(importStream,
				new Reader.ReaderImportRowHandler() {
					Map<String, String> columnMap = scheduleImport
							.getDefaultColumnMap();
					String frequencyColumn = columnMap
							.get(FREQUENCY_DEFAULT_COLUMN_HEADER);
					String startTimeColumn = columnMap
							.get(START_TIME_DEFAULT_COLUMN_HEADER);
					String endTimeColumn = columnMap
							.get(END_TIME_DEFAULT_COLUMN_HEADER);
					String durationTimeColumn = columnMap
							.get(DURATION_DEFAULT_COLUMN_HEADER);
					String dateColumn = columnMap
							.get(DATE_DEFAULT_COLUMN_HEADER);
					String endsColumn = columnMap
							.get(ENDS_DEFAULT_COLUMN_HEADER);
					String intervalColumn = columnMap
							.get(INTERVAL_DEFAULT_COLUMN_HEADER);
					String repeatColumn = columnMap
							.get(REPEAT_DEFAULT_COLUMN_HEADER);
						...
						while (columnIterator.hasNext()) {
							...
							if (value.length() == 0) {
								...
								if (frequencyColumn != null && frequencyColumn.equals(column.getColumnHeader())) {
									...
									if (!success) {
										try {
											mapCellValue = timeFormatterWithSeconds()
													.parse(value);
											success = true;
										}
									...
									if (!success) {
										try {
											mapCellValue = time24HourFormatter()
													.parse(value);
											success = true;
										}
									...
									if (!success) {
										try {
											mapCellValue = time24HourFormatterWithSeconds()
													.parse(value);
											success = true;
										}
										...
										&& durationTimeColumn.equals(column
												.getColumnHeader())) {
									String timeFormatErrorString = (String) rb
											.getFormattedMessage(
													"err_time",
													new Object[] {
															Integer.valueOf(column
									...
									if (parts.length == 1) {
										...
										// property field.
										try {
											mapCellValue = Integer.valueOf(Integer
													.parseInt(parts[0]));
										} catch (NumberFormatException ex) {
											throw new ImportException(
													timeFormatErrorString);
										}
										...
										&& dateColumn.equals(column
												.getColumnHeader())
										|| (endsColumn != null && endsColumn
												.equals(column
														.getColumnHeader()))) {
									DateFormat df = DateFormat.getDateInstance(
											DateFormat.SHORT, rb.getLocale());
									df.setLenient(false);
									try {
								...
								} else if (intervalColumn != null
										&& intervalColumn.equals(column
												.getColumnHeader())
										|| repeatColumn != null
										&& repeatColumn.equals(column
												.getColumnHeader())) {
									try {
										mapCellValue = Integer.valueOf(column
												.getCellValue());
									} catch (NumberFormatException ex) {
										String msg = (String) rb
												.getFormattedMessage(
								...
								} else if (ITEM_TYPE_PROPERTY_NAME
										.equals(column.getColumnHeader())) {
							...
							eventProperties.put(column.getColumnHeader(),
									mapCellValue);
						}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.sakaiproject.tool.calendar.schedimport.importers.Importer#
