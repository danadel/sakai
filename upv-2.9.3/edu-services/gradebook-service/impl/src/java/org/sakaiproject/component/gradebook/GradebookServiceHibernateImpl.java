/**********************************************************************************
*
* $Id: GradebookServiceHibernateImpl.java 317994 2015-03-16 08:11:55Z anueda@asic.upv.es $
*
***********************************************************************************
*
		...
		getHibernateTemplate().execute(new HibernateCallback() {
			public Object doInHibernate(Session session) throws HibernateException {
				...
				if (clientServiceDescription != null) {
					// Registramos el evento para que se active la CR
					postUpdateGradeEvent(gradebookUid, assignment.getId(), studentUid, convertStringToDouble(score));
				}
