/**********************************************************************************
*
* $Id: GradebookExternalAssessmentServiceImpl.java 319874 2015-06-19 07:51:34Z anueda@asic.upv.es $
*
***********************************************************************************
*
...
public class GradebookExternalAssessmentServiceImpl extends BaseHibernateManager implements GradebookExternalAssessmentService {
    ...
    // SAK-27494: marked as synchronized	--bbailla2
	...
	public Long getExternalAssignmentId(final String gradebookUid, final String externalId) throws GradebookNotFoundException {
		...
		public Object doInHibernate(Session session) throws HibernateException {
			...
			for (Iterator iter = existingScores.iterator(); iter.hasNext(); ) {
				...
				if ( alwaysUpdate || (newPointsEarned != null && !newPointsEarned.equals(oldPointsEarned)) || (newPointsEarned == null && oldPointsEarned != null) ) {
					...
					postUpdateGradeEvent(gradebookUid, assignment.getId(), studentUid, newPointsEarned);
				}
			}
			...
			public Object doInHibernate(Session session) throws HibernateException {
				...
				for (Iterator iter = previouslyUnscoredStudents.iterator(); iter.hasNext(); ) {
					...
					if (newPointsEarned != null) {
						...
						postUpdateGradeEvent(gradebookUid, assignment.getId(), studentUid, convertStringToDouble(newPointsEarned));
					}
				}
		...
		HibernateCallback hc = new HibernateCallback() {
			public Object doInHibernate(Session session) throws HibernateException {
				...
				if ( alwaysUpdate || (newPointsEarned != null && !newPointsEarned.equals(oldPointsEarned)) ||
            		...
            		postUpdateGradeEvent(gradebookUid, asn.getId(), studentUid, newPointsEarned);
				} else {
					if(log.isDebugEnabled()) log.debug("Ignoring updateExternalAssessmentScore, since the new points value is the same as the old");
				}
				return null;
	...
	private void postUpdateGradeEvent(String gradebookUid, Long assignmentId, String studentUid, Double pointsEarned) {
	    if (eventTrackingService != null) {
            eventTrackingService.postEvent("gradebook.updateItemScore","/gradebook/"+gradebookUid+"/"+assignmentId+"/"+studentUid+"/"+pointsEarned+"/student"); 
        }
	}
