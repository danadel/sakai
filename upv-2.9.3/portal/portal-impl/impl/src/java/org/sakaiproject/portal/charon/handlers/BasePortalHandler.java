/**********************************************************************************
 * $URL: https://source.sakaiproject.org/svn/msub/upv.es/PoliformaT/upv-2.9.3/portal/portal-impl/impl/src/java/org/sakaiproject/portal/charon/handlers/BasePortalHandler.java $
 * $Id: BasePortalHandler.java 313858 2014-09-25 12:30:18Z anueda@asic.upv.es $
 ***********************************************************************************
 *
...
public abstract class BasePortalHandler implements PortalHandler
{
	...
	protected void addLocale(Session session, Site site) {
		Locale locale = setSiteLanguage(site);	
		if(log.isDebugEnabled()) {
			log.debug("Locale for site " + site.getId() + " = " + locale.toString());
		}
        
		String sessionAttr = ResourceLoader.LOCALE_KEY + session.getUserId();
		session.setAttribute(sessionAttr, locale);
	}
}
