...
alter table vc_rooms_cnn 
	add index FK_vc_cnn_s (site_id), 
	add constraint FK_vc_cnn_s foreign key (site_id) references SAKAI_SITE (SITE_ID) ON DELETE CASCADE ON UPDATE CASCADE;
	
alter table vc_groups_cnn 
	add index FK_vc_cnn_g_g (group_id),
	add constraint FK_vc_cnn_g_g foreign key (group_id) references SAKAI_SITE_GROUP (GROUP_ID) ON DELETE CASCADE ON UPDATE CASCADE;
...
alter table vc_users_cnn 
	add index FK_vc_cnn_u_u (user_id),
	add constraint FK_vc_cnn_u_u foreign key (user_id) references SAKAI_USER (USER_ID) ON DELETE CASCADE ON UPDATE CASCADE;
