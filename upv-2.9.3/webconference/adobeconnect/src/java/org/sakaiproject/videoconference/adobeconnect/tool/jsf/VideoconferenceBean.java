package org.sakaiproject.videoconference.adobeconnect.tool.jsf;
...
public class VideoconferenceBean {	
	...
	public TimeZone getUserTimeZone(){
		return I18nService.getInstance().getTimeZone();
	}
	...
	{
		return videoconferenceLogic.hasPermission(ExternalLogic.PER_CNN_CREATE_ROOM);
	}
	
	public boolean getCanEdit()
	{
		return videoconferenceLogic.hasPermission(ExternalLogic.PER_CNN_EDIT_ROOM);
	}
	
	public boolean getCanDelete()
	{
		return videoconferenceLogic.hasPermission(ExternalLogic.PER_CNN_DELETE_ROOMS);
	}
	
	public boolean getCanAccess()
	{
		return videoconferenceLogic.hasPermission(ExternalLogic.PER_CNN_ACCESS_ROOM);
	}
	
	public boolean getCanModerate()
	{
		return videoconferenceLogic.hasPermission(ExternalLogic.PER_CNN_MODERATE_ROOM);
	}
	
	public boolean getCanCheckConnectivity()
	{
		return videoconferenceLogic.hasPermission(ExternalLogic.PER_CNN_CHECK_CONNECTIVITY);
	}
	...
	public String processViewURL()
	{
		if(getCanAccess() || getCanModerate())
		{
			currentRoom = (VideoconferenceRoomWrapper) roomsModel.getRowData();
			return "viewURL";
		}
	...
	public List<SelectItem> getTypeOptions()
	{
		if(typeOptions == null)
		{
			List<VideoconferenceRoomType> list = videoconferenceLogic.getRoomTypes();
			if(!list.isEmpty())
			{
				typeOptions = new ArrayList<SelectItem>();
				//typeOptions.add(new SelectItem("", ""));
				
				for(VideoconferenceRoomType rt : list)
					typeOptions.add(new SelectItem(rt.getId(), rt.getName()));
			}
		}		
	...
	public List<SelectItem> getPublicOptions()
	{
		List<SelectItem> ret = new ArrayList<SelectItem>();
		ret.add(new SelectItem(true, I18nService.getInstance().getMessage("public_room") + "(" + getUrl() + ")"));
		//ret.add(new SelectItem(false, I18nService.getInstance().getMessage("protected_room")));
	...
	private boolean validateFields()
	{
		...
		{
			message = I18nService.getInstance().getMessage("max_lenght_name");
			fc.addMessage("items", new FacesMessage(FacesMessage.SEVERITY_INFO, message, message));
			ret = false;			
		}
	...
	private String processErrors()
	{
		...
		{
			if(msg.getSummary().equals(MessageList.ERR_VC) || msg.getSummary().equals(MessageList.ERR_DUPLICATED_NAME))
			{
				FacesContext fc = FacesContext.getCurrentInstance();
