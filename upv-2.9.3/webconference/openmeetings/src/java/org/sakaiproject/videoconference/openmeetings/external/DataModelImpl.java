...
{	
	...
	private static final String SQL_TABLE = "vc_rooms_om";
	...
	private static final String SQL_SEQUENCE = "vc_sec_om";
	private static final String SQL_SEQUENCE_VALUE = "value";

	private static final String SQL_GROUP_TABLE = "vc_groups_om";
	private static final String SQL_GROUPID = "group_id";

	private static final String SQL_USER_TABLE = "vc_users_om";
	...
	{		
		...
		if(Boolean.valueOf(externalLogic.getSakaiProperty(ExternalLogic.AUTODDL)))
		{
			try
			{
				sqlService.ddl(this.getClass().getClassLoader(), "sakai_videoconference_openmeetings");				
			}
			catch(Exception e)
			{
	...
	/* (non-Javadoc)
	 * @see org.sakaiproject.videoconference.external.DataModel#getSiteRooms(java.lang.String)
	 */
		...
		try
		{
			...
			if(ret)
			{
				...
				if(!room.isAccessBySite())
				{
					...
					//a�adimos los usuarios asociados
	...
	//obtiene el valor actual de la secuencia e incrementa su valor
	@SuppressWarnings("unchecked")
	public int getAndUpdateSequence()
	{
		...
		try
		{
			...
			{			
				...
				String sql = "SELECT "+SQL_SEQUENCE_VALUE+" FROM "+SQL_SEQUENCE;
				List<String> l = sqlService.dbRead(sql);
				boolean bln = false;
				...
				if(l.isEmpty())
				{
					...
					//el valor es 2, puesto que este proceso es equivalente a obtener el primer valor (1) e incrementarlo
					sql = "INSERT INTO "+SQL_SEQUENCE+" ("+SQL_SEQUENCE_VALUE+") VALUES (2)";
					bln = sqlService.dbWrite(sql);
					if(bln)
						ret = 1;
				}
				else
				{
					...
					sql = "UPDATE "+SQL_SEQUENCE+" SET "+SQL_SEQUENCE_VALUE+" = "+SQL_SEQUENCE_VALUE+" + 1";
					bln = sqlService.dbWrite(sql);
	
					if(bln)
						ret = Integer.parseInt(l.get(0));
				}
			}else if(vendor.equals("oracle"))
			{
				String sql = "SELECT "+SQL_SEQUENCE+".NEXTVAL FROM DUAL";
				List<String> l = sqlService.dbRead(sql);
				ret = Integer.parseInt(l.get(0));
			}
