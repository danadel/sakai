			...
			<h:form id="roomListForm">
				...
				<h:dataTable
					...
					<h:column>
						...
						<h:commandLink id="accessLink" action="#{VideoconferenceBean.processAccessRoom}" rendered="#{VideoconferenceBean.checkLogin && VideoconferenceBean.canAccess && entry.room.active}">							
							<h:outputText value="#{entry.room.name}"/>
						</h:commandLink>
						<h:outputText value="#{entry.room.name}" rendered="#{!VideoconferenceBean.checkLogin || !VideoconferenceBean.canAccess || !entry.room.active}"/>
					</h:column>
					
					<h:column rendered="#{VideoconferenceBean.canEdit}">
						<f:facet name="header">
							<h:outputText value="#{msgs.edit_room}"/>
						</f:facet>
						<h:commandLink id="updateLink" action="#{VideoconferenceBean.processEditRoom}" rendered="#{VideoconferenceBean.checkLogin && entry.room.active}">							
							<h:outputText value="#{msgs.edit_room}"/>
						</h:commandLink>
						<h:outputText value="#{msgs.edit_room}" rendered="#{!VideoconferenceBean.checkLogin || !entry.room.active}"/>
					</h:column>
					
					<h:column>
						<f:facet name="header">
							<h:outputText value="#{msgs.room_capacity}"/>
						</f:facet>						
						<h:outputText value="#{entry.room.capacity}"/>
					</h:column>
