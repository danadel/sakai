...
public class SkinnableLogin extends HttpServlet implements Login {
		...
		throws ServletException, IOException 
	{
		...
		else if ("/xlogin".equals(option))
		{
			option = "/login";
			skipContainer = true;
			xloginPDA= (Boolean) session.getAttribute("isPDA");
		}
		...
		//UPV: Se ha movido este codigo delante del control de containerlogin para poder pasar el parametro isPDA a la llamda al containerLogin
		// PDA or not?
		...
		//UPV ponemos esta info en la sesión para saber si entró o no con el parámetro isPDA
        session.setAttribute("isPDA",isPDA);
		...
		if (checkContainer && !skipContainer)
		{
			...
			if (session.getAttribute(ATTR_CONTAINER_CHECKED) == null)
			{
                ...
                if (isPDA) {containerCheckUrl=containerCheckUrl+"&isPDA=true";};
				/*
	...
	protected void complete(String returnUrl, Session session, Tool tool, HttpServletResponse res) throws IOException
	{
		...
		//UPV: Recuperamos la variable que pusimos para saber que era PDA y sustituimos el URL para que incluya el pda
		if (session != null) {
        Boolean isPDA = (Boolean) session.getAttribute("isPDA");
        if (isPDA) { returnUrl = returnUrl.replace("/portal","/portal/pda"); };
        };
		
		//Codigo UPV para forzar a que la redireccion vaya a un top
		String pagina="<script language=\"Javascript\">  top.location = \"" + res.encodeRedirectURL(returnUrl) + "\"</script> ";
		log.info("UPV: Redirigiendo a Top ...:"+pagina);
