...
// WARNING: keep in sync with code in AjaxFilter.java
	...
	public String getItemGroupTitles(String itemGroups, SimplePageItem item) {
		...
		if (group != null) {
			...
			//it might be a user
	    	try {
				User user = UserDirectoryService.getUser(groupIds[i]);
				groupNames.add(user.getDisplayName());
			} catch(Exception e){
				//it isn't an user, neither
				groupNames.add(messageLocator.getMessage("simplepage.deleted-group"));
			}
		}
	    }
    ...
    // the group list, so you need to do i.setGroups().
	   ...
	   if (lessonEntity != null) {
	       ...
	       List<String>newGroups = new ArrayList<String>(Arrays.asList(groups));
	       for (int n = 0; n < oldGroups.size(); n++) {
		       if (!newGroups.contains(oldGroups.get(n))) {
		    	   //it might be a user
		    	   try {
		    		   User user = UserDirectoryService.getUser(oldGroups.get(n));
		    		   newGroups.add(oldGroups.get(n));
		    	   } catch(Exception e){}
		       }
		   }
