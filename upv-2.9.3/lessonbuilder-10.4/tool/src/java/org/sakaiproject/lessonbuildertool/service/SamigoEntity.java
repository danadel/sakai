...
public class SamigoEntity implements LessonEntity, QuizEntity {
  ...
  // hack for forums. not used for assessments, so always ok
	...
	if (!control.getReleaseTo().equals(AssessmentAccessControlIfc.RELEASE_TO_SELECTED_STUDENTS)) {
	    control.setReleaseTo(AssessmentAccessControlIfc.RELEASE_TO_SELECTED_STUDENTS);
...
// I think it's cheaper to get the best assessment, since we don't actually care whether it's 1 or >= 1.
	...
	if (groups.size() == 1 && groups.get(0).equals(siteId))
		...
		AuthzQueriesFacadeAPI authz = PersistenceService.getInstance().getAuthzQueriesFacade();
  		List authorizations = authz.getAuthorizationByFunctionAndQualifier("TAKE_ASSESSMENT", assessment.getPublishedAssessmentId().toString());
	...
	if (groups == null && !control.getReleaseTo().equals(AssessmentAccessControlIfc.RELEASE_TO_SELECTED_STUDENTS))
	    return;
