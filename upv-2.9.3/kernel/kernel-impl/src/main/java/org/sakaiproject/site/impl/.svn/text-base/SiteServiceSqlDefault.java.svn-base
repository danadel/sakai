...
public class SiteServiceSqlDefault implements SiteServiceSql
{
	...
	{
		return "insert into SAKAI_SITE_GROUP (GROUP_ID, SITE_ID, TITLE, DESCRIPTION, SECTION_STATE) values (?,?,?,?,?)";
	}
	...
	{
		return "select SS.GROUP_ID, SS.TITLE, SS.DESCRIPTION, SS.SECTION_STATE " + "from SAKAI_SITE_GROUP SS where SS.SITE_ID = ?";
	}
