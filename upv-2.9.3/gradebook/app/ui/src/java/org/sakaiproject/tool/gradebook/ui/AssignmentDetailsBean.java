/**********************************************************************************
*
* $Id: AssignmentDetailsBean.java 306749 2014-03-04 15:24:04Z anueda@asic.upv.es $
*
***********************************************************************************
*
...
public class AssignmentDetailsBean extends EnrollmentTableBean {
	...
	public void processUpdateScores(ActionEvent event) {
		try {
			for (AssignmentGradeRecord agr : updatedGradeRecords) {
				getGradebookBean().getEventTrackingService().postEvent("gradebook.updateItemScore","/gradebook/"+getGradebookUid()+"/"+agr.getAssignment().getId()+"/"+agr.getStudentId()+"/"+agr.getPointsEarned()+"/"+getAuthzLevel());
			}
			saveScores();
	...
	/*           EXPORTAR A EXCEL LAS NOTAS DE LOS ALUMNOS Y LOS CAMBIOS EN ELLAS              */
	...
	private List<List<Object>> getSpreadsheetData() {
		...
		headerRow.add(getLocalizedString("assignment_details_points"));
		headerRow.add(getLocalizedString("assignment_details_by"));
		headerRow.add(getLocalizedString("assignment_details_date"));  
		headerRow.add(getLocalizedString("assignment_details_comments_read"));  
	        
	        
		spreadsheetData.add(headerRow);
	
		for (Object scorer : scoreRows){
			ScoreRow scoreRow = (ScoreRow)scorer;
			User student = scoreRow.getEnrollment().getUser();
			String comment = scoreRow.getCommentText();
			List scoreEvents = scoreRow.getEventRows();
			if ((scoreEvents!=null) && (scoreEvents.size()>0)){
				Object event = scoreEvents.get(scoreEvents.size()-1);
				List<Object> row = new ArrayList<Object>();        	
				row.add(student.getDisplayId());
				row.add(student.getSortName());
				row.add(((GradingEventRow)event).getGrade());
				row.add(((GradingEventRow)event).getGraderName());
				row.add(((GradingEventRow)event).getDate());	        	
				row.add(comment);
		        	
				//Add the row to the spreasheet
				spreadsheetData.add(row);
			}else{
				List<Object> row = new ArrayList<Object>();        	
				//Students has no score
				row.add(student.getDisplayId());
				row.add(student.getSortName());
				row.add("-");
				row.add("-");
