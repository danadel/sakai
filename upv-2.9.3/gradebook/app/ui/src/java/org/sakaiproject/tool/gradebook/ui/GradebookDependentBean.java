/**********************************************************************************
*
* $Id: GradebookDependentBean.java 318149 2015-03-26 16:37:28Z anueda@asic.upv.es $
*
***********************************************************************************
*
...
public abstract class GradebookDependentBean extends InitializableBean {
    ...
    private String urlPadrino;
    private String urlPadrino3;
	...
	public PublishedAssessmentService getPublishedAssessmentService() {
		return getGradebookBean().getPublishedAssessmentService();
	}
	...
	/**
	 * Get the url to Padrino
	 * @return
	 */
	public String getUrlPadrino() {
		String url = "https://docente.upv.es/padrino/padrino.jsp";
		String params[] = {"asi", "dni", "idioma", "caca", "url"};
		String values[] = {"", "", "", "", "https://docente.upv.es/padrino/"};
		StringBuffer urlPadrino = new StringBuffer(url);
		...
		// get the language
		String idioma = "C";
		ResourceLoader resource = new ResourceLoader("Messages");
		...
		for (int i=0; i<params.length; i++) {
			if (i == 0) urlPadrino.append('?');
			else urlPadrino.append('&');
			urlPadrino.append(params[i]);
			urlPadrino.append('=');
			urlPadrino.append(values[i]);
		}
				
		return urlPadrino.toString();
	}
	
	/**
	 * Get the url to Padrino v3
	 * @return
	 */
	public String getUrlPadrino3() {
		String url = "https://aplicat.upv.es/padrino-app/index.html";
		String params[] = {"asi", "caca"};
		String values[] = {"", ""};
		StringBuffer urlPadrino = new StringBuffer(url);
		...
		try {
			...
			ResourceProperties prop = site.getProperties();
			values[0] = prop.getProperty("asiupv");
			values[1] = prop.getProperty("term");
	...
	/**
	 * Return if it shows the Padrino link 
	 * @return
	 */
	public Boolean getRenderUrl() {
		...
		try {
			...
			ResourceProperties prop = site.getProperties();
			String renderUrl = prop.getProperty("propietario");
			if (renderUrl != null && 
				(renderUrl.equalsIgnoreCase("alu") || renderUrl.equalsIgnoreCase("doc") || renderUrl.equalsIgnoreCase("gra"))) {
