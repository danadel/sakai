/**********************************************************************************
*
* $Id: RemoveAssignmentBean.java 317907 2015-03-06 12:39:45Z anueda@asic.upv.es $
*
***********************************************************************************
*
...
public class RemoveAssignmentBean extends GradebookDependentBean implements Serializable {
    ...
    private boolean hasItemsLinked;
    private List<String>itemsLinkedToGradebook = new ArrayList<String>();
    ...
    public String removeAssignment() {
    	setHasItemsLinked(false);
    	
    	if(removeConfirmed) {
        	List<String> items = new ArrayList<String>();
        	
        	List<Notification> notifications = NotificationService.findNotifications("gradebook.updateItemScore", "/gradebook/"+getGradebookUid()+"/"+assignmentId);
        	...
        	while (iter.hasNext()) {
        		...
        		if (itemName != null)
        			items.add(toolName + "; " + itemName);
        		else
        			items.add(toolName);
        	}
        	
        	if (notifications.size() > 0) {
        		setItemsLinkedToGradebook(items);
        		setHasItemsLinked(true);
        		return null;
        	}
	...
	public List<String> getItemsLinkedToGradebook() {
		return itemsLinkedToGradebook;
	}

	public void setItemsLinkedToGradebook(List<String> itemsLinkedToGradebook) {
		this.itemsLinkedToGradebook = itemsLinkedToGradebook;
	}

	public boolean isHasItemsLinked() {
		return hasItemsLinked;
	}

	public void setHasItemsLinked(boolean hasItemsLinked) {
		this.hasItemsLinked = hasItemsLinked;
	}
}
