...
public class AssignmentBean extends GradebookDependentBean implements Serializable {
	...
	public String updateAssignment() {
		try {
			...
			// Check if there are assessments which send grades to this gradebook item
			// If yes, we cannot allow to modify the gradebook item's name
			List<PublishedAssessmentFacade> publishedAssessments = getPublishedAssessmentService().
					getBasicInfoOfAllPublishedAssessments("title", true, getGradebookUid());
			for (PublishedAssessmentFacade publishedAssessment : publishedAssessments) {
				PublishedAssessmentFacade facade = (PublishedAssessmentFacade) getPublishedAssessmentService().
						getPublishedAssessment(publishedAssessment.getPublishedAssessmentId());
				String gradebookName = facade.getAssessmentMetaDataByLabel(AssessmentMetaDataIfc.TO_GRADEBOOK_NAME);
				if (originalAssignment.getName().equals(gradebookName) && !originalAssignment.getName().equals(assignment.getName())) {
					FacesUtil.addErrorMessage(getLocalizedString("edit_assignment_error_title_assessment") + facade.getTitle());
		            return "failure";
				}
			}
			...
			// If yes, we cannot allow to modify the gradebook item's name
			Iterator iAssignments = AssignmentService.getAssignmentsForContext(getGradebookUid());
			...
			while (iAssignments.hasNext())
			{
				...
				if (originalAssignment.getName().equals(associateGradebookAssignment) && !originalAssignment.getName().equals(this.assignment.getName())) {
			...
			getGradebookBean().getEventTrackingService().postEvent("gradebook.updateAssignment","/gradebook/"+getGradebookUid()+"/"+assignment.getId()+"/"+assignment.getPointsPossible()+"/"+dueDateMillis+"/"+assignment.isReleased()+"/"+assignment.isCounted()+"/"+getAuthzLevel());
