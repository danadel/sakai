...
//    		getHibernateTemplate().initialize(oldForum.getTopicsSet());
  ...
  public void sendPrivateMessage(PrivateMessage message, Map<User, Boolean> recipients, boolean asEmail)
  {
    ...
    Map<String, PrivateForum> pfMap = null;
    	...
    	if (siteTitle!=null)
    		additionalHeaders.add("Subject: [" + serverName + "] " + siteTitle + ": " + message.getTitle());
    	else
    		additionalHeaders.add("Subject: [" + serverName + "]: " + message.getTitle());
	...
	if(forwardingEnabled)
	{
		...
		HashMap<RecipientType, InternetAddress[]> recipientsf = new HashMap<RecipientType, InternetAddress[]>();
		recipientsf.put(RecipientType.BCC, fAddressesArr);
		emailService.sendMail(new InternetAddress(systemEmail), fAddressesArr, message.getTitle(), 
				bodyString, recipientsf, null, additionalHeaders, null);
    }
  ...
  private String buildMessageBody(PrivateMessage message) {
	  ...
	  String footer = "<p>-------------------------<br>" +
	  getResourceBundleString("pvt_email_click") + " <a href=\"mailto:" + currentUser.getEmail() + "\">" + currentUser.getEmail() + "</a>" +
	  getResourceBundleString("pvt_email_warning") + "<br>" +
	  "-------------------------<br>" +
