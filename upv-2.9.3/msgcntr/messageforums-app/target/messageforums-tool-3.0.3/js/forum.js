...
document.getElementsByTagName('head').item(0).appendChild(js);*/
function doAjaxAll(topicId, self){
	$(self).attr('src', '/library/image/sakai/spinner.gif');
	$.ajax({ type: "GET", url: document.forms[0].action , data: "ajax=true&action=markMessageAsRead&messageId=all&topicId=" + topicId,
		success: function(msg){
			if(msg.match(/SUCCESS/)){
	     		setTimeout(function(){
	     						$("span.childrenNew").hide();
	 							$("span.messageNew").hide(); 							
	 							$("span.unreadMsg").removeClass("unreadMsg");
	 							$("img.markAsReadIcon").remove();
	 							$("span.markAsReadIcon").remove();
	 							$("span.textPanelFooter").each(function() {
									$(this).html($(this).html().replace(/ - [0-9][0-9]* /," - 0 "));
	 							});
	 							$(self).remove();
	            }, 500);
	         } else {
