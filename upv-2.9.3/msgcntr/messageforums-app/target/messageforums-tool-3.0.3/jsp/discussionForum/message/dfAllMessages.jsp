		...
		<sakai:tool_bar separator="#{msgs.cdfm_toolbar_separator}">
      	...
      	<sakai:tool_bar_item value="#{msgs.cdfm_flat_view}" action="#{ForumTool.processActionDisplayFlatView}" />
				...
				<h:outputLink id="print" value="javascript:printFriendly('#{ForumTool.printFriendlyUrl}');">
					...
					<h:outputText  value=" | " rendered="#{ForumTool.selectedTopic.unreadNoMessages > 0}" />
				    <h:outputLink title="#{msgs.cdfm_mark_as_read}" rendered="#{ForumTool.selectedTopic.unreadNoMessages > 0}"
				    	 	onclick="doAjaxAll(#{ForumTool.selectedTopic.topic.id}, this);return false;">
				    	 <h:outputText  value="#{msgs.cdfm_mark_all_as_read} "/>
				    	 <h:graphicImage value="/../../library/image/silk/email_open.png" alt="#{msgs.cdfm_mark_as_read}" title="#{msgs.cdfm_mark_as_read}" styleClass="markAsReadIcon"/>
				    </h:outputLink>				
 		</sakai:tool_bar>
			...
			<h:column>
				...
				<h:panelGroup rendered="#{!message.deleted}" >
					<h:outputText value="#{message.message.created}" rendered="#{message.read}">
						<f:convertDateTime pattern="#{msgs.date_format_us}" />
					</h:outputText>
