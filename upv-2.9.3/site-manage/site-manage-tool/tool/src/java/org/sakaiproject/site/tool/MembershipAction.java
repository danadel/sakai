/**********************************************************************************
 * $URL: https://source.sakaiproject.org/svn/msub/upv.es/PoliformaT/upv-2.9.3/site-manage/site-manage-tool/tool/src/java/org/sakaiproject/site/tool/MembershipAction.java $
 * $Id: MembershipAction.java 313898 2014-09-26 07:17:08Z vacercon@upvnet.upv.es $
 ***********************************************************************************
 *
...
public class MembershipAction extends PagedResourceActionII
{
	...
	protected int sizeResources(SessionState state)
	{
		...
		//UPV: Anyadimos un filtro por propiedades si estamos en el site OPENUFASU, OPENPDI
		HashMap<String,String> searchprop = new HashMap();
		if (ToolManager.getCurrentPlacement().getContext().equals("OPENUFASU")) {
			searchprop.put("epigrafe", "type.upv.oufa");
			searchtiposite="sitecfp";
		} else if (ToolManager.getCurrentPlacement().getContext().equals("OPENPDI")) {
			searchprop.put("epigrafe", "type.upv.opdi");
			searchtiposite="sitecfp";
        } else {
        	searchprop=null;
        	searchtiposite=null;
		...
		if (defaultMode)
		{
		List unjoinableSites = SiteService.getSites(org.sakaiproject.site.api.SiteService.SelectionType.ACCESS, searchtiposite,
							 search, searchprop, org.sakaiproject.site.api.SiteService.SortType.TITLE_ASC, null);
		...
		List openSites = SiteService.getSites(org.sakaiproject.site.api.SiteService.SelectionType.JOINABLE,
				searchtiposite, search, searchprop, org.sakaiproject.site.api.SiteService.SortType.TITLE_ASC, null);
	...
	protected List readResourcesPage(SessionState state, int first, int last)
	{
		...
		if (defaultMode)
		{
			if (sortAsc)
			{
				rv = SiteService.getSites(org.sakaiproject.site.api.SiteService.SelectionType.ACCESS, searchtiposite, search,
						searchprop, org.sakaiproject.site.api.SiteService.SortType.TITLE_ASC, page);
			}
			else
			{
				rv = SiteService.getSites(org.sakaiproject.site.api.SiteService.SelectionType.ACCESS, 
                                                searchtiposite, search,
						searchprop, org.sakaiproject.site.api.SiteService.SortType.TITLE_DESC, page);
			}
		}
		else
		{
			...
			if (sortAsc)
			{
				rv = SiteService.getSites(org.sakaiproject.site.api.SiteService.SelectionType.JOINABLE,
						searchtiposite, search, searchprop, org.sakaiproject.site.api.SiteService.SortType.TITLE_ASC, page);
			}
			else
			{
				rv = SiteService.getSites(org.sakaiproject.site.api.SiteService.SelectionType.JOINABLE,
						searchtiposite, search, searchprop, org.sakaiproject.site.api.SiteService.SortType.TITLE_DESC, page);
			}
		}
	...
	public String buildMainPanelContext(VelocityPortlet portlet, Context context, RunData rundata, SessionState state)
	{
		...
		//UPV: ponemos por defecto que entre en sites-joinables la primera vez
		if (state.getAttribute(SORT_ASC) == null)
		{	
