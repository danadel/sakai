 ...
 * $URL: https://source.sakaiproject.org/svn/msub/upv.es/PoliformaT/upv-2.9.3/site-manage/site-manage-tool/tool/src/java/org/sakaiproject/site/tool/SiteAction.java $
 * $Id: SiteAction.java 130929 2013-10-28 22:43:34Z rmengod@cc.upv.es $
 ***********************************************************************************
 *
...
public class SiteAction extends PagedResourceActionII {
	...
	private String buildContextForTemplate(String preIndex, int index, VelocityPortlet portlet,
		...
		case 10:
			...
			context.put("epigrafe", siteInfo.epigrafe);
		...
		case 12:
			...
			try {
				...
				// Epigrafe
				String siteTypeUpv = getSiteTypeUpv(site.getType());
				context.put("isTypeUpv", siteTypeUpv);
				
				String epigrafe = siteProperties.getProperty(Site.PROP_SITE_EPIGRAFE);
				if (epigrafe != null) {
					context.put("epigrafe", rb.getString(epigrafe));
				}
		...
		case 13:
			...
			context.put("isTypeUpv", getSiteTypeUpv(siteInfo.site_type));
			context.put("selectedEpigrafe", siteInfo.epigrafe);
			context.put("epigrafes", rb.getStrings("site.type.upv"));
		...
		case 14:
			...
			context.put("isTypeUpv", getSiteTypeUpv(siteInfo.site_type));
			context.put("epigrafe", siteInfo.epigrafe);
			context.put("oEpigrafe", siteProperties.getProperty(Site.PROP_SITE_EPIGRAFE));			
	...
	public void doGet_site(RunData data) {
		...
		if (!h.containsKey(siteId)) {
			...
			// 50 as default
			state.setAttribute(STATE_PAGESIZE, Integer.valueOf(50));

			// update
			h.put(siteId, Integer.valueOf(50));
	...
	*/
	...
	public void doSave_siteInfo(RunData data) {
		...
		// Epigrafe
		String epigrafe = siteInfo.epigrafe;
		if (epigrafe != null) {
			siteProperties.addProperty(Site.PROP_SITE_EPIGRAFE, epigrafe);
		}
	...
	void doUpdate_site_access_joinable(RunData data,
		...
		String paramValue= (String) state.getAttribute(STATE_JOINABLE);
		Boolean joinable = state.getAttribute(STATE_JOINABLE) != null ? ((Boolean) state.getAttribute(STATE_JOINABLE)) : null;
		if (!sEdit.isPublished())
		{
			// reset joinable role if the site is not published
			sEdit.setJoinable(false);
			sEdit.setJoinerRole(null);
		} else if ((joinable!=null) && joinable){
			...
			if (ServerConfigurationService.getBoolean(CONVERT_NULL_JOINABLE_TO_UNJOINABLE, true)){
				sEdit.setJoinable(false);
				sEdit.setJoinerRole(null);				
			}
		} else if (!joinable) {
	...
	private void updateSiteInfo(ParameterParser params, SessionState state) {
		...
		if (params.getString("description") != null) {
			...
			FormattedText.processFormattedText(description, alertMsg, true, false, true);
			String[] alertMsg_linea = alertMsg.toString().split(String.valueOf('\n')); 
			
			if (alertMsg.toString().contains("iframe") && alertMsg_linea.length <=2) {
				siteInfo.description = FormattedText.processFormattedText(description, alertMsg, false, false, true);
			}
			else {
				siteInfo.description = FormattedText.processFormattedText(description, alertMsg, true, false, true);
			}
		}
		...
		String epigrafe = params.getString("epigrafe");
		siteInfo.epigrafe = epigrafe;
	...
	private void addNewSite(ParameterParser params, SessionState state) {
		...
		if (state.getAttribute(STATE_MESSAGE) == null) {
			try {
				...
				// UPV: epigrafe
				rp.addProperty(Site.PROP_SITE_EPIGRAFE, siteInfo.epigrafe);
	...
	public class SiteInfo {
		...
		public String epigrafe = NULL_STRING; // Epigrafe
	...
	//UPV: Funcion para saber el tipo de site UPV
	private String getSiteTypeUpv(String type) {
		if ("siteupv".equals(type) || "siteford".equals(type) || 
				"sitemas".equals(type) || "sitecfp".equals(type)) {
			return "true";
		}
		return "false";
	}
