...
(function()
{
	...
	CKEDITOR.dialog.prototype =
	{
		...
		layout : function()
		{
			var editorsTextArea = CKEDITOR.document.getByClassName('cke_skin_v2');
			for (index=0; index<editorsTextArea.length; index++) {
				var editor=editorsTextArea[index];
						
				if (CKEDITOR.currentInstance != null
					&& ('cke_' + CKEDITOR.currentInstance.name) == editor.id)
				{
					this.move (editor.getBoundingClientRect().left + 50,  
			   			editor.getBoundingClientRect().top + 25);
					break;
				}
			}
