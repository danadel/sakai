...
package org.sakaiproject.tool.assignment.audio;
...
public class AudioRecorder extends JPanel implements ActionListener,
	...
	private JPanel makeSaveTFPanel() {
		...
		if (params.getAttemptsRemaining() >= 9999) {
			saveTFpanel.add(rtextField = new JTextField(res.getString("unlimited")));
...
//		JPanel buttonsPanel = new JPanel(new SpringLayout());
	...
	*/
	...
	public void saveAndPost(InputStream inputStream,
		...
		Thread saveAndPostThread = new Thread() {
			public void run() {
				...
				if (post){
					boolean result = postAudio(audioType, urlString);
					
					/*Samoo*/
					if (containingApplet != null) {
						JSObject window = JSObject.getWindow(containingApplet);
						
						if(result){
							//Enable html input button save
							window.eval("document.getElementById('post').disabled = false");
							//Show upload ok html message
							window.eval("document.getElementById('upload-ok').style.display = 'block'");
						}else{
							//Show upload error html message
							window.eval("document.getElementById('upload-error').style.display = 'block'");
						}
						
						//hide applet html element
						window.eval("document.getElementById('audio-applet').style.display = 'none'");
					}					
				}
			} // end of run
	...
	public boolean postAudio(AudioFileFormat.Type audioType, String urlString) {
		...
		try {
			...
			try {
				...
				if("ok".equalsIgnoreCase(response)){
					reportStatus("Server upload response : " + response + "\n");
					return true;
				}else{
					reportStatus("Server media saving error\n");
					return true;
				}
			} catch (Exception ex) {
				reportStatus("Server upload error : " + ex.toString());
			}
			finally{
		      	if (input !=null){
		    		try {
