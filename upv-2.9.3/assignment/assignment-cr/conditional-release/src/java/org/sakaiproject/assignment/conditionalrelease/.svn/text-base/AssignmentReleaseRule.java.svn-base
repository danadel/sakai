 ...
 *
 **********************************************************************************/
package org.sakaiproject.assignment.conditionalrelease;
...
/**
 * @author zach, adapted by samoo
 *
 */
public class AssignmentReleaseRule implements Rule, Obsoletable {
	...
	private static Log log = LogFactory.getLog(AssignmentReleaseRule.class);
	...
	private ConditionService conditionService = (ConditionService) ComponentManager.get("org.sakaiproject.assignment.conditionalrelease.AssignmentConditionService");
	...
	public AssignmentReleaseRule() {

	}
	
	public AssignmentReleaseRule(String resourceId, List<Condition> predicates, Conjunction conj) {
	...
	public void notify(Notification notification, Event event) {
		...
		if(notification.getProperties().getProperty("AssociatedTool")!=null){//TODO ramificar para Samigo / tareas
			
			if(((String)notification.getProperties().getProperty("AssociatedTool")).equals("sakai.assignment.grades")){

				AssignmentEdit a = null;

				log.debug("en ARR, id -> " + this.resourceId);

				try{
					a = AssignmentService.editAssignment(this.resourceId);
				} catch (Exception ee){
					log.debug("error al cargar la tarea!!");
				}
				
				if(a!=null){
					ResourcePropertiesEdit aPropertiesEdit = a.getPropertiesEdit();
					
					String[] tokens = this.resourceId.split("\\/");
					String siteId = "/site/" + tokens[3];
					if (("gradebook.updateItemScore").equals(event.getEvent())) {
						...
						try {
							...
							aPropertiesEdit.removeProperty(SATISFIES_RULE);//si quitas una que no existe que pasa??
	
							List<String> prop = null;
							if(aPropertiesEdit.getProperty("conditional.access.list")!=null)
								prop = Arrays.asList( ((String)aPropertiesEdit.getProperty("conditional.access.list")).replaceAll("\\[|\\]| ","").split(",") );
							...
							if (!shouldBeAvailable && acl.contains(grading.getUserId())) {
								...
								aPropertiesEdit.addProperty("conditional.access.list", (new ArrayList(acl)).toString());
						...
						try {
							aPropertiesEdit.addProperty(SATISFIES_RULE, String.valueOf(shouldBeAvailable));
							AssignmentService.commitEdit(a);
						} catch(Exception e){
							e.printStackTrace();
						} finally {
						...
						try{ 
							...
							AuthzGroup group = authzGroupService.getAuthzGroup(siteId);
							...
							// since we're following a per-user event now, the global rule property should be removed
							aPropertiesEdit.removeProperty(SATISFIES_RULE);
							aPropertiesEdit.addProperty("conditional.access.list",acl.toString());
							AssignmentService.commitEdit(a);
	...
	public void set(NotificationAction other) {
		AssignmentReleaseRule eOther = (AssignmentReleaseRule) other;
