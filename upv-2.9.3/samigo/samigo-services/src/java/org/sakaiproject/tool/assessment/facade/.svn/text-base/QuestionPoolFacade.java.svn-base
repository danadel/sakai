    ...
    return ownerDisplayName;
  	...
  	public String getCanAddPools() {
		String result = "false";
  		...
  		Set<QuestionPoolAccessData> access = getQuestionPoolAccess();
  		Iterator iter = access.iterator();
  		while (iter.hasNext()) {
  			QuestionPoolAccessData qpad = (QuestionPoolAccessData)iter.next();
  			if (qpad.getAgentId().equals(AgentFacade.getAgentString())) {
  				Long accessTypeId = qpad.getAccessTypeId();
  				result = 
  					(accessTypeId.equals(QuestionPoolDataFacade.READ_WRITE) ||
  					accessTypeId.equals(QuestionPoolDataFacade.ADMIN)? "true" : "false");
  				break;
  			}
  		}
		...
		String result = "false";
  		...
  		while (iter.hasNext()) {
  			...
  			if (qpad.getAgentId().equals(AgentFacade.getAgentString())) {
  				Long accessTypeId = qpad.getAccessTypeId();
  				result = 
  					(accessTypeId.equals(QuestionPoolDataFacade.READ_ONLY) ||
  					accessTypeId.equals(QuestionPoolDataFacade.MODIFY) ||
  					accessTypeId.equals(QuestionPoolDataFacade.READ_WRITE) ||
  					accessTypeId.equals(QuestionPoolDataFacade.ADMIN)? "true" : "false");
  				break;
  			}
