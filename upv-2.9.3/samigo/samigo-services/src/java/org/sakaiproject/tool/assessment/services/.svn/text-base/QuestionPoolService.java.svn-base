...
public class QuestionPoolService
{
  ...
  public QuestionPoolIteratorFacade getAllPoolsWithAccess(String agentId, Long access)
  {
	  ...
	  results =
		  (QuestionPoolIteratorFacade) PersistenceService.getInstance().
		  getQuestionPoolFacadeQueries().getAllPoolsWithAccess(agentId, access);
	  return results;
  }
...
//TODO must call the Service.
  ...
  */
  ...
  public void removeQuestionPoolAccess(Tree tree, String user, Long questionPoolId) {
	  try {
		  PersistenceService.getInstance().
		  getQuestionPoolFacadeQueries().removeQuestionPoolAccess(tree, user, questionPoolId);
	  } catch (Exception e) {
		  log.error(e);
	  }
  }

  public List<QuestionPoolDataFacade> getAgentsWithAccess(Long questionPoolId) {

	  List<QuestionPoolDataFacade> qpds = null;
	  try {
		  qpds = PersistenceService.getInstance().
  ...
  public List<QuestionPoolDataFacade> getAgentsWithoutAccess(Long questionPoolId, String realmId) {

	  List<QuestionPoolDataFacade> agents = new ArrayList<QuestionPoolDataFacade>();

	  try {
		  // Get agents with access
		  List<QuestionPoolDataFacade> agentsWithAccess = getAgentsWithAccess(questionPoolId);
		  ...
		  while (iter.hasNext()) {
			  QuestionPoolDataFacade qpd = (QuestionPoolDataFacade)iter.next();
			  
			  users.remove(qpd.getAgentId());
		  }
		  
		  // Create the QuestionPoolAccessFacade with users
		  for (String userId : users) {
			  QuestionPoolAccessData qpd = new QuestionPoolAccessData(questionPoolId, userId, QuestionPoolDataFacade.ACCESS_DENIED); 
			  agents.add(new QuestionPoolDataFacade(qpd));
		  }
  	...
  	{
  		QuestionPoolAccessData qpaccess = null;
  		try
  		{
  			qpaccess =
  				PersistenceService.getInstance().getQuestionPoolFacadeQueries().
  					getQuestionPoolAccessData(poolId, agentId);
  		}
  		catch(Exception e)
  		{
  			log.error(e); throw new RuntimeException(e);
