    ...
    implements java.io.Serializable, PublishedAssessmentIfc, Cloneable
{
  ...
  private boolean removedForDisplay;
  ...
  private void setProperties(PublishedAssessmentIfc data){
    ...
    if (data.getAssessmentAccessControl() != null) {
    	this.dueDate = data.getAssessmentAccessControl().getDueDate();
    	this.startDate = data.getAssessmentAccessControl().getStartDate();
    	this.releaseTo = data.getAssessmentAccessControl().getReleaseTo();
    }
  }
...
).equals(SectionDataIfc.RANDOM_DRAW_FROM_QUESTIONPOOL.toString())))
{
  ...
  public boolean getRemovedForDisplay() {
	  return this.removedForDisplay;
  }
  
  public void setRemovedForDisplay(boolean removedForDisplay) {
	  this.removedForDisplay = removedForDisplay;
  }
}
