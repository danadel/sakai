...
public class AssessmentService {
	 ...
	 */
	public ArrayList getBasicInfoOfAllAssessments(String orderBy,
			boolean ascending) {
		String siteAgentId = AgentFacade.getCurrentSiteId();
		return PersistenceService.getInstance().getAssessmentFacadeQueries()
				.getBasicInfoOfAllAssessmentsByAgent(false, orderBy,
	...
	public ArrayList getBasicInfoOfAllActiveAssessments(String orderBy,
		...
		String siteAgentId = AgentFacade.getCurrentSiteId();
		return PersistenceService.getInstance().getAssessmentFacadeQueries()
				.getBasicInfoOfAllAssessmentsByAgent(true, orderBy,
						siteAgentId, ascending); // signalling all & no
													// paging
	}
	...
	public AssessmentFacade restoreAssessment(Long assessmentId, String newTitle) {
		AssessmentFacade restored = PersistenceService.getInstance().getAssessmentFacadeQueries()
				.restoreAssessment(assessmentId, newTitle);
	  	return restored;
	}
	...
	public ContentResource createCopyOfContentResource(String resourceId,
		...
		try {
			...
			ContentResource cr = AssessmentService.getContentHostingService().findResource(resourceId);
			if (cr != null) {
	...
	/**
	 * Share Templates with other user
	 */
	public void addTemplateAccess(Long templateId, String user) {
		try {
			PersistenceService.getInstance().
				getAssessmentFacadeQueries().addTemplateAccess(templateId, user);
		} catch (Exception e) {
			log.error(e);
		}
	}
	...
	public void removeTemplateAccess(Long templateId, String user) {
		try {
			PersistenceService.getInstance().
			getAssessmentFacadeQueries().removeTemplateAccess(templateId, user);
		} catch (Exception e) {
			log.error(e);
		}
	}
	public List<AssessmentTemplateSharedFacade> getAgentsWithAccess(Long templateId, String ownerId) {

		List<AssessmentTemplateSharedFacade> sharedTemplates = null;
		try {
			sharedTemplates = PersistenceService.getInstance().
					getAssessmentFacadeQueries().getTemplatesShared(templateId, ownerId);
		} catch (Exception e) {
			log.error(e);
		}
	...
	public List<AssessmentTemplateSharedFacade> getAgentsWithoutAccess(Long templateId, String ownerId, String realmId) {

		List<AssessmentTemplateSharedFacade> agents = new ArrayList<AssessmentTemplateSharedFacade>();

		try {
			// Get agents with access
			List<AssessmentTemplateSharedFacade> agentsWithAccess = getAgentsWithAccess(templateId, ownerId);

			List<String> azGroups = new ArrayList<String>();
			azGroups.add("/site/" + realmId);

			// Get the site's users 
			Set<String> users = AuthzGroupService.getUsersIsAllowed("assessment.template.create", azGroups);

			// Delete the site's users who have already got access
			Iterator iter = agentsWithAccess.iterator();
			while (iter.hasNext()) {
				AssessmentTemplateSharedFacade ats = (AssessmentTemplateSharedFacade)iter.next();
				  
				users.remove(ats.getAgentId());
			}
			  
			// Create the AssessmentTemplateSharedFacade with users
			for (String userId : users) {
				AssessmentTemplateSharedData atsd = new AssessmentTemplateSharedData(templateId, userId); 
				agents.add(new AssessmentTemplateSharedFacade(atsd));
			}	
