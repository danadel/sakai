<!-- $Id: poolTreeTable.jsp 131823 2013-11-21 12:57:50Z anueda@asic.upv.es $
<%--
***********************************************************************************
*
...
<h:graphicImage id="spacer" style="border:0" width="30" height="14" value="/images/delivery/spacer.gif" />
 ...
 <!-- Add Pool -->
 <h:commandLink title="#{questionPoolMessages.t_addSubpool}" rendered="#{questionpool.importToAuthoring != 'true' && authorization.createQuestionPool && pool.canAddPools}"  styleClass="tier#{questionpool.tree.currentLevel}" id="addlink" immediate="true" action="#{questionpool.addPool}">
  <h:outputText id="add" value="#{questionPoolMessages.add}"/>
  <f:param name="qpid" value="#{pool.questionPoolId}"/>
</h:commandLink>
<!-- Copy Pool -->
<h:outputText rendered="#{questionpool.importToAuthoring != 'true' && authorization.copyOwnQuestionPool && pool.canCopyPools && pool.canAddPools}" value=" #{questionPoolMessages.separator} " />
...
<!-- Move Pool -->
<h:outputText rendered="#{questionpool.importToAuthoring != 'true' && authorization.editOwnQuestionPool && pool.canMovePools && pool.canCopyPools}" value=" #{questionPoolMessages.separator} " />
...
<!-- Share Pool -->
<h:outputText rendered="#{questionpool.importToAuthoring != 'true' && authorization.editOwnQuestionPool && pool.ownerId==questionpool.agentId && pool.parentPoolId == 0 && pool.canMovePools}" value=" #{questionPoolMessages.separator} " />
...
<!-- Unshare Pool -->
<h:outputText rendered="#{questionpool.importToAuthoring != 'true' && authorization.copyOwnQuestionPool && pool.canCopyPools && pool.ownerId!=questionpool.agentId && pool.parentPoolId == 0}" value=" #{questionPoolMessages.separator} " />
<h:commandLink title="#{questionPoolMessages.t_unsharePool}" rendered="#{questionpool.importToAuthoring != 'true' && authorization.copyOwnQuestionPool && pool.canCopyPools && pool.ownerId!=questionpool.agentId && pool.parentPoolId == 0}" id="unsharelink" immediate="true" action="#{questionpoolshare.unsharePool}">
  <h:outputText id="unshare" value="#{questionPoolMessages.t_unsharePool}"/>
  <f:param name="qpid" value="#{pool.questionPoolId}"/>
</h:commandLink>
