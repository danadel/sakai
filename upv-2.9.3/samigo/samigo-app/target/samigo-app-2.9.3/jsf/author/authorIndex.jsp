...
<!--
* $Id: authorIndex.jsp 316695 2015-01-12 12:22:16Z anueda@asic.upv.es $
<%--
**********************************************************************************
*
...
function initTabAssessment() {
	$("#authorIndexForm\\:assessment-check-removed").attr('checked', false);
	$("#assessment-published-limiter").show();
	$("#authorIndexForm\\:coreAssessments tr").show();
	$("#authorIndexForm\\:coreAssessments tbody tr span.removed_true").parent().parent().hide();
	$("#assessment-published-limiter a").removeClass('active');
	$("#assessment-link-all").addClass('active');
}

function initTabPublished() {
	$("#authorIndexForm\\:published-check-removed").attr('checked', false);
	$("#published-status-limiter").show();
	$("#authorIndexForm\\:published-assessments tr").show();
	$("#authorIndexForm\\:published-assessments tbody tr span.removed_true").parent().parent().hide();
	$("#published-status-limiter a").removeClass('active');
	$("#published-link-status-all").addClass('active');
...
$(document).ready(function() {
	...
	<h:outputText rendered="#{!author.justPublishedAnAssessment}" value="initTabAssessment();" />
	<h:outputText rendered="#{author.justPublishedAnAssessment}" value="initTabPublished();" />
	...
	// ALLOW ALL, PUBLISHED, NON PUBLISHED LINKS FOR ASSESSMENT ITEMS
	$("#assessment-link-all").click(
		function() {
			$("#authorIndexForm\\:coreAssessments tr").show();
			$("#authorIndexForm\\:coreAssessments tbody tr span.removed_true").parent().parent().hide();
			$("#assessment-published-limiter a").removeClass('active');
	...
	$("#assessment-link-published").click(
		function() {
			$("#authorIndexForm\\:coreAssessments tbody tr").show();
			$("#authorIndexForm\\:coreAssessments tbody tr span.title_false").parent().parent().hide();
			$("#authorIndexForm\\:coreAssessments tbody tr span.removed_true").parent().parent().hide();
			$("#assessment-published-limiter a").removeClass('active');
			$(this).addClass('active');
		}
	);
	$("#assessment-link-unpublished").click(
		function() {
			$("#authorIndexForm\\:coreAssessments tbody tr").show();
			$("#authorIndexForm\\:coreAssessments tbody tr span.title_true").parent().parent().hide();
			$("#authorIndexForm\\:coreAssessments tbody tr span.removed_true").parent().parent().hide();
			$("#assessment-published-limiter a").removeClass('active');
			$(this).addClass('active');
		}
	...
	$("#published-link-status-all").click(
		function() {
			$("#authorIndexForm\\:published-assessments tr").show();
			$("#authorIndexForm\\:published-assessments tbody tr span.removed_true").parent().parent().hide();
			$("#published-status-limiter a").removeClass('active');
	...
	$("#published-link-status-active").click(
		function() {
	...
	$("#published-link-status-inactive").click(
		function() {
			$("#authorIndexForm\\:published-assessments tbody tr").show();
	...
	$("#authorIndexForm\\:assessment-check-removed").click(
		function() {
			if ($(this).is(':checked')) {
				$("#authorIndexForm\\:coreAssessments tr").show();
				$("#authorIndexForm\\:coreAssessments tbody tr span.removed_false").parent().parent().hide();
				$("#assessment-published-limiter").hide();
			}
			else {
				$("#assessment-published-limiter").show();
				$("#authorIndexForm\\:coreAssessments tr").show();
	...
	$("#authorIndexForm\\:published-check-removed").click(
		function() {
			if ($(this).is(':checked')) {
				$("#authorIndexForm\\:published-assessments tr").show();
				$("#authorIndexForm\\:published-assessments tbody tr span.removed_false").parent().parent().hide();
				$("#published-status-limiter").hide();
				$("#authorIndexForm\\:assessment-retracted").hide();
			}
			else {
				$("#published-status-limiter").show();
...
<div id="tabs">
 ...
 <h:outputText escape="false" rendered="#{authorization.createAssessment}" value="<div id=\"tabs-1\">"/>
  <div id="assessment-removed-limiter">
  	<h:outputText value="#{authorFrontDoorMessages.assessment_only_removed}" />
	<h:selectBooleanCheckbox id="assessment-check-removed" ></h:selectBooleanCheckbox>
  </div>
  
  <div id="assessment-published-limiter">
    <span><h:outputText value="#{authorFrontDoorMessages.assessment_view}" />:</span>
    <span id="assessment-status-all">
      <a class="active" id="assessment-link-all"><h:outputText value="#{authorFrontDoorMessages.assessment_view_all}" /></a>
    </span>
    <span class="separator">|</span>
    <span id="assessment-status-published">
      <a id="assessment-link-published"><h:outputText value="#{authorFrontDoorMessages.assessment_status_published}" /></a>:
    </span>
    <span><h:outputText value="#{authorFrontDoorMessages.assessment_status_published_sub}" /></span>
    <span class="separator">|</span>
    <span id="assessment-status-unpublished">
      <a id="assessment-link-unpublished"><h:outputText value="#{authorFrontDoorMessages.assessment_status_unpublished}" /></a>:
    </span>
    <span><h:outputText value="#{authorFrontDoorMessages.assessment_status_unpublished_sub}" /></span>
  </div>
  ...
  <t:dataTable cellpadding="0" cellspacing="0" rowClasses="list-row-even,list-row-odd" styleClass="tablesorter" id="coreAssessments" value="#{author.assessments}" var="coreAssessment" rendered="#{authorization.adminCoreAssessment}" summary="#{authorFrontDoorMessages.sum_coreAssessment}">
    <t:column headerstyleClass="selectAction" styleClass="selectAction">
	  ...
	  <h:selectOneMenu id="pendingSelectAction1" value="select" onchange="clickPendingSelectActionLink(this);" rendered="#{!coreAssessment.removedForDisplay && coreAssessment.questionSize > 0 }">
	  ...
	  <h:selectOneMenu id="pendingSelectAction2" value="select" onchange="clickPendingSelectActionLink(this);" rendered="#{!coreAssessment.removedForDisplay && coreAssessment.questionSize == 0 }">
		<f:selectItems value="#{author.pendingSelectActionList2}" />
		<f:valueChangeListener	type="org.sakaiproject.tool.assessment.ui.listener.author.ActionSelectListener" />
	  </h:selectOneMenu>
	  
	  <h:selectOneMenu id="pendingSelectAction3" value="select" onchange="clickPendingSelectActionLink(this);" rendered="#{coreAssessment.removedForDisplay}">
		<f:selectItems value="#{author.pendingSelectActionList3}" />
		<f:valueChangeListener	type="org.sakaiproject.tool.assessment.ui.listener.author.ActionSelectListener" />
	  </h:selectOneMenu>
	  ...
	  <h:commandLink id="pendingHiddenlink3" action="#{author.getOutcome}" value="" >
	    <f:param name="editType" value="pendingAssessment" />
        <f:param name="assessmentId" value="#{coreAssessment.assessmentBaseId}"/>
	  </h:commandLink>
	</t:column>
    ...
    <t:column headerstyleClass="titlePending" styleClass="titlePending">
      ...
      <f:verbatim><span class="title_</f:verbatim><h:outputText value="#{coreAssessment.publishedForDisplay}" /><f:verbatim>"></f:verbatim>
    ...
    <t:column headerstyleClass="lastModified" styleClass="lastModified">
	  ...
	  <f:verbatim><span class="removed_</f:verbatim><h:outputText value="#{coreAssessment.removedForDisplay}" /><f:verbatim>"></f:verbatim>
...
<div id="tabs-2">
  <div id="published-removed-limiter">
  	<h:outputText value="#{authorFrontDoorMessages.assessment_only_removed}" />
	<h:selectBooleanCheckbox id="published-check-removed" ></h:selectBooleanCheckbox>
  </div>
  <div id="published-status-limiter">
    <span><h:outputText value="#{authorFrontDoorMessages.assessment_view}" />:</span>
    <span id="published-status-all">
      <a class="active" id="published-link-status-all"><h:outputText value="#{authorFrontDoorMessages.assessment_view_all}" /></a>
    </span>
    <span class="separator">|</span>
    <span id="published-status-active">
      <a id="published-link-status-active"><h:outputText value="#{authorFrontDoorMessages.assessment_status_active}" /></a>:
    </span>
    <span><h:outputText value="#{authorFrontDoorMessages.assessment_status_active_sub}" /></span>
    <span class="separator">|</span>
    <span id="published-status-inactive">
      <a id="published-link-status-inactive"><h:outputText value="#{authorFrontDoorMessages.assessment_status_inactive}" /></a>:
    </span>
  ...
  <t:dataTable id="published-assessments" rowClasses="list-row-even,list-row-odd" cellpadding="0" cellspacing="0" styleClass="tablesorter" rendered="#{authorization.adminPublishedAssessment}"
    ...
    <t:column headerstyleClass="selectAction" styleClass="selectAction">
	  ...
	  <h:selectOneMenu id="publishedSelectAction1" value="select" onchange="clickPublishedSelectActionLink(this);" rendered="#{!publishedAssessment.removedForDisplay && (author.isGradeable && publishedAssessment.submittedCount > 0) && (author.isEditable && (!author.editPubAssessmentRestricted || !publishedAssessment.hasAssessmentGradingData))}">
	  ...
	  <h:selectOneMenu id="publishedSelectAction2" value="select" onchange="clickPublishedSelectActionLink(this);" rendered="#{!publishedAssessment.removedForDisplay && (author.isGradeable && publishedAssessment.submittedCount > 0) && !(author.isEditable && (!author.editPubAssessmentRestricted || !publishedAssessment.hasAssessmentGradingData))}">
		<f:selectItem itemLabel="#{authorMessages.select_action}" itemValue="select"/>
		<f:selectItem itemLabel="#{authorMessages.action_scores}" itemValue="scores"/>
		<f:selectItems value="#{author.publishedSelectActionList}" />
		<f:valueChangeListener	type="org.sakaiproject.tool.assessment.ui.listener.author.ActionSelectListener" />
	  </h:selectOneMenu>
	  <h:selectOneMenu id="publishedSelectAction3" value="select" onchange="clickPublishedSelectActionLink(this);" rendered="#{!publishedAssessment.removedForDisplay && !(author.isGradeable && publishedAssessment.submittedCount > 0) && (author.isEditable && (!author.editPubAssessmentRestricted || !publishedAssessment.hasAssessmentGradingData))}">
		<f:selectItem itemLabel="#{authorMessages.select_action}" itemValue="select"/>
		<f:selectItem itemLabel="#{commonMessages.edit_action}" itemValue="edit_published"/>
		<f:selectItems value="#{author.publishedSelectActionList}" />
		<f:valueChangeListener	type="org.sakaiproject.tool.assessment.ui.listener.author.ActionSelectListener" />
	  </h:selectOneMenu>
	  <h:selectOneMenu id="publishedSelectAction4" value="select" onchange="clickPublishedSelectActionLink(this);" rendered="#{!publishedAssessment.removedForDisplay && !(author.isGradeable && publishedAssessment.submittedCount > 0) && (author.isEditable && !(!author.editPubAssessmentRestricted || !publishedAssessment.hasAssessmentGradingData))}">
		<f:selectItem itemLabel="#{authorMessages.select_action}" itemValue="select"/>
		<f:selectItems value="#{author.publishedSelectActionList}" />
		<f:valueChangeListener	type="org.sakaiproject.tool.assessment.ui.listener.author.ActionSelectListener" />
	  </h:selectOneMenu>
	  <h:selectOneMenu id="publishedSelectAction5" value="select" onchange="clickPublishedSelectActionLink(this);" rendered="#{publishedAssessment.removedForDisplay}">
		<f:selectItem itemLabel="#{authorMessages.select_action}" itemValue="select"/>
		<f:selectItems value="#{author.publishedSelectActionList2}" />
		<f:valueChangeListener	type="org.sakaiproject.tool.assessment.ui.listener.author.ActionSelectListener" />
	  </h:selectOneMenu>
	...
	<t:column headerstyleClass="titlePub" styleClass="titlePub">
      <f:facet name="header">
		...
		<f:verbatim><span class="removed_</f:verbatim><h:outputText value="#{publishedAssessment.removedForDisplay}" /><f:verbatim>"></f:verbatim>
      	<h:outputText id="publishedAssessmentTitle2" value="#{publishedAssessment.title}" />
      	<h:outputText value="#{authorFrontDoorMessages.asterisk_2}" 
      		rendered="#{publishedAssessment.status == 3 && !publishedAssessment.removedForDisplay}" styleClass="validate"/>
      	<f:verbatim></span></f:verbatim> 
    </t:column>
	...
	<t:column headerstyleClass="status" styleClass="status">
	 ...
	 <h:panelGroup>
	  <f:verbatim><span class="status_</f:verbatim><h:outputText value="#{publishedAssessment.activeStatus}" /><f:verbatim>"></f:verbatim>
	    <h:outputText value="#{authorFrontDoorMessages.assessment_status_active}" 
	    	rendered="#{publishedAssessment.activeStatus==true && !publishedAssessment.removedForDisplay}"/>
	    <h:outputText value="#{authorFrontDoorMessages.assessment_status_inactive}" 
	    	rendered="#{publishedAssessment.activeStatus==false && !publishedAssessment.removedForDisplay}"/>
	  <f:verbatim></span></f:verbatim>
     </h:panelGroup>
	</t:column>
	...
	<t:column headerstyleClass="inProgress" styleClass="inProgress">
	 ...
	 <h:panelGroup>
	  <h:outputText value="#{publishedAssessment.inProgressCount}" rendered="#{!publishedAssessment.removedForDisplay}"/>
     </h:panelGroup>
	</t:column>
	...
	<t:column headerstyleClass="submitted" styleClass="submitted">
	 ...
	 <h:panelGroup rendered="#{!publishedAssessment.removedForDisplay}">
	...
	<t:column headerstyleClass="releaseTo" styleClass="releaseTo">
	  ...
	  <h:panelGroup rendered="#{!publishedAssessment.removedForDisplay}">
	      ...
	      <h:outputText value="#{authorFrontDoorMessages.entire_site}" rendered="#{publishedAssessment.releaseTo ne 'Anonymous Users' && publishedAssessment.releaseTo ne 'Selected Groups' && publishedAssessment.releaseTo ne 'Assign Students'}" />
	      <h:outputText value="#{authorFrontDoorMessages.selected_groups}" rendered="#{publishedAssessment.releaseTo eq 'Selected Groups'}"/>
	      <h:outputText value="#{authorFrontDoorMessages.adhoc_groups}" rendered="#{publishedAssessment.releaseTo eq 'Assign Students'}"/>
		  <f:verbatim><span class="info"></f:verbatim>
		  <h:graphicImage id="infoImg" url="/images/info_icon.gif" alt="" rendered="#{publishedAssessment.releaseTo eq 'Selected Groups' || publishedAssessment.releaseTo eq 'Assign Students'}" styleClass="infoDiv"/>
		  ...
		  <t:dataTable var="releaseToGroups" styleClass="makeInfo" style="display:none;z-index:2000;" value="#{publishedAssessment.releaseToGroupsList}" 
			rendered="#{publishedAssessment.releaseTo eq 'Selected Groups' || publishedAssessment.releaseTo eq 'Assign Students'}">
			<t:column>
    ...
    <t:column headerstyleClass="releaseDate" styleClass="releaseDate">
      ...
      <h:outputText value="#{publishedAssessment.startDate}" rendered="#{!publishedAssessment.removedForDisplay}" >
	...
	<t:column headerstyleClass="dueDate" styleClass="dueDate">
      ...
      <h:outputText value="#{publishedAssessment.dueDate}" rendered="#{!publishedAssessment.removedForDisplay}" >
          <f:convertDateTime pattern="#{generalMessages.output_date_picker}"/>
      </h:outputText>
    </t:column>
