/**********************************************************************************
 * $URL: https://source.sakaiproject.org/svn/msub/upv.es/PoliformaT/upv-2.9.3/samigo/samigo-app/src/java/org/sakaiproject/tool/assessment/ui/servlet/delivery/LoginServlet.java $
 * $Id: LoginServlet.java 321699 2015-11-03 16:52:55Z anueda@asic.upv.es $
 ***********************************************************************************
 *
 ...
 * @author Ed Smiley
 * @version $Id: LoginServlet.java 321699 2015-11-03 16:52:55Z anueda@asic.upv.es $
 */
      ...
      throws ServletException, IOException
  {
    ...
    boolean isAuthenticated = false;
      ...
      if (releaseTo != null && releaseTo.indexOf("Anonymous Users")> -1){
        ...
        if (isAuthenticated){
          if (releaseTo.indexOf(AssessmentAccessControl.RELEASE_TO_SELECTED_STUDENTS)>-1) {
      ...
      // Check: in case the assessment is Conditional Released, user has to satisfy its rule
      boolean agentNotSatisfiedCR = false; 
      Boolean condRelease = pub.getAssessmentAccessControl().isUseConditionalRelease();
      if (condRelease != null && condRelease.booleanValue()) {
	      if (pub.getAssessmentAccessControl().isSatisfiesRule() != null) {
	    	  if (!pub.getAssessmentAccessControl().isSatisfiesRule()) {
	    		  agentNotSatisfiedCR = true;
	    	  } 
	      } else if (pub.getAssessmentAccessControl().getConditionalAccessList() != null) {
	    	  if (!pub.getAssessmentAccessControl().getConditionalAccessList().contains(agentIdString)) {
	    		  agentNotSatisfiedCR = true;
	    	  } 
	      }
      }
