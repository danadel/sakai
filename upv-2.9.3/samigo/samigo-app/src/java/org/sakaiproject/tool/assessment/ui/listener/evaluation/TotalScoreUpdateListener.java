/**********************************************************************************
 * $URL: https://source.sakaiproject.org/svn/msub/upv.es/PoliformaT/upv-2.9.3/samigo/samigo-app/src/java/org/sakaiproject/tool/assessment/ui/listener/evaluation/TotalScoreUpdateListener.java $
 * $Id: TotalScoreUpdateListener.java 133130 2014-01-09 15:51:11Z anueda@asic.upv.es $
 ***********************************************************************************
 *
 ...
 * @author Ed Smiley
 * @version $Id: TotalScoreUpdateListener.java 133130 2014-01-09 15:51:11Z anueda@asic.upv.es $
 */
  ...
  implements ActionListener
{
  ...
  public boolean saveTotalScores(TotalScoresBean bean)
  {
      ...
      StringBuffer idList = new StringBuffer(" ");
  	  ...
  	  if(bean.getSelectedEvaluationMode().equals("1")){
  		  while (iter.hasNext())
  		  {
  			  AgentResults agentResults = (AgentResults) iter.next();
  		...
  		try{
  			String newScore = bean.getGlobalTotalScore();
  			String newOverrideScore = bean.getGlobalTotalOverrideScore();
  			String newComments = bean.getGlobalComments();
  			float totalScore = 0; 
  			float totalOverrideScore = 0;
  			boolean changeTotal = false;
  			boolean changeOverride = false;
  			boolean changeComments = false;
  			if (newScore != null) {
  				if (!("").equals(newScore.trim()) && !("-").equals(newScore.trim())) {
  					totalScore = Float.valueOf(newScore).floatValue();
  					changeTotal = true;
  				}
  			}
  			if (newOverrideScore != null) {
  				if (!("").equals(newOverrideScore.trim()) && !("-").equals(newOverrideScore.trim())) {
  					totalOverrideScore = Float.valueOf(newOverrideScore).floatValue();
  					changeOverride = true;
  				}
  			}
  			if (newComments != null && !("").equals(newComments.trim()))
  				changeComments = true;
  		  			
  			Collection allAgents = bean.getAllAgents();
  			iter = allAgents.iterator();
  		  						
  			AuthzQueriesFacadeAPI authz = PersistenceService.getInstance().getAuthzQueriesFacade();
  			List authorizations = authz.getAuthorizationByFunctionAndQualifier("TAKE_PUBLISHED_ASSESSMENT", bean.getPublishedAssessment().getPublishedAssessmentId().toString());
  		  						
  			while(iter.hasNext()){
  				AgentResults agentResults = (AgentResults) iter.next();
  				AssessmentGradingData data = new AssessmentGradingData();
  		  							
  				//filtering only the assigned students
  				String currentSiteId = AgentFacade.getCurrentSiteId();	
  				ArrayList groupIds = PersistenceService.getInstance().getPublishedAssessmentFacadeQueries().getSiteGroupIdsForSubmittingAgent(agentResults.getIdString(), currentSiteId);
  				...
  				if (authorizations != null && authorizations.size()>0) {		
  					...
  					while (authsIter.hasNext()) {
  						...
  						if (agentResults.getIdString().equals(ad.getAgentIdString()) || (groupIds!=null && groupIds.contains(ad.getAgentIdString()))){
  							can_take = true;
  							break;
  						}
  					}
  				...
  				if(changeTotal)
  					agentResults.setTotalAutoScore(newScore);
  				else
  					totalScore = Float.valueOf(agentResults.getTotalAutoScore());
  				data.setTotalAutoScore(totalScore);						
  		  			
  				if(changeOverride)
  					agentResults.setTotalOverrideScore(newOverrideScore);
  				else if(!("-").equals(agentResults.getTotalOverrideScore()))
  					totalOverrideScore = Float.valueOf(agentResults.getTotalOverrideScore());
  				data.setTotalOverrideScore(totalOverrideScore);
  		  								
  				if(changeComments)
  					agentResults.setComments(newComments);
  				else 
  					newComments = agentResults.getComments();					
  				data.setComments(agentResults.getComments());
  				
  				BeanUtils.copyProperties(data, agentResults);
  				data.setPublishedAssessmentId(bean.getPublishedAssessment().getPublishedAssessmentId());
  				data.setFinalScore(totalScore + totalOverrideScore);
  				data.setIsLate(agentResults.getIsLate());
  				data.setGradedBy(AgentFacade.getAgentString());
  				data.setGradedDate(new Date());
  			...
  			FacesContext context = FacesContext.getCurrentInstance();
  			String err2 = (String) ContextUtil.getLocalizedString("org.sakaiproject.tool.assessment.bundle.EvaluationMessages", "number_format_error_collective");
  			context.addMessage(null,  new FacesMessage(err2));
  			return true;
  		}
  	  }
