/**********************************************************************************
 * $URL: https://source.sakaiproject.org/svn/msub/upv.es/PoliformaT/upv-2.9.3/samigo/samigo-app/src/java/org/sakaiproject/tool/assessment/ui/bean/author/AssessmentSettingsBean.java $
 * $Id: AssessmentSettingsBean.java 321328 2015-09-25 07:50:37Z anueda@asic.upv.es $
 ***********************************************************************************
 *
...
//private String editComponents; // 0 = cannot
  ...
  private SelectItem[] existingGradebook;
  ...
  private String gradebookItem;
  private String conditionalOptionSelected;
  private boolean checkConditionalRelease = false;
  //private boolean useConditionalRelease = false;
  private String conditionArgument;
  private String notificationId;
  private String submittedFunctionName;
  private boolean renderConditionArgument = false;
    ...
    try {
      ...
      if (accessControl != null) {
        ...
        try {
        	...
        	for(Iterator iter = available.iterator(); iter.hasNext();) {
        		...
        		User user = UserDirectoryService.getUser(student);
        		//if(!AgentFacade.getRole(user.getId()).equals("access"))
        		if (!SecurityService.unlock(user,CAN_TAKE, "/site/"+site.getId())||user.getId().equals("admin"))
        			continue;
        		if(!serialized_userList.equals("")){	
        			SelectItem item = new SelectItem(user.getId(), user.getSortName());
        			if(!serialized_userList.contains(user.getId()))
        				userList_available.add(item);
        		} else
        			userList_available.add(new SelectItem(user.getId(), user.getSortName()));
        	}
        			
        	groupList_available = new ArrayList<SelectItem>();
        	Collection availableGroups = site.getGroups();
        	if(availableGroups!=null && availableGroups.size()>0)
        		hasAvailableGroups = true;
        	else
        		hasAvailableGroups = false;
        	for(Iterator iter = availableGroups.iterator(); iter.hasNext();) {
        		Group group = (Group) iter.next();
        		String grDescription = "";
        		if(group.getDescription()!=null)
        			grDescription = " : " + group.getDescription();
        		if(!serialized_groupList.equals("")){					
        			SelectItem item = new SelectItem(group.getId(), group.getTitle() + grDescription);
        			if(!serialized_groupList.contains(group.getId()))
        				groupList_available.add(item);
        		} else
        			groupList_available.add(new SelectItem(group.getId(), group.getTitle() + grDescription));
        	}
        	
        } catch (Exception ex) {
        	// No site or user available
        }
      ...
      if (evaluation != null) {
        ...
        if (evaluation.getToGradeBook()!=null ) {
        	...
        	if (evaluation.getToGradeBook().equals(EvaluationModelIfc.TO_SELECTED_GRADEBOOK.toString()))
        		this.gradebookName = assessment.getAssessmentMetaDataByLabel(AssessmentMetaDataIfc.TO_GRADEBOOK_NAME);
  ...
  protected void populateSelectedLists() {
	serialized_groupList = "";
	serialized_userList = "";
	userList_selected = new ArrayList<SelectItem>();
  	groupList_selected = new ArrayList<SelectItem>();
  	if(releaseTo!=null && releaseTo.equals(AssessmentAccessControlIfc.RELEASE_TO_SELECTED_STUDENTS)){
  		...
  		if (authorizations != null && authorizations.size()>0) {		
  			Iterator authsIter = authorizations.iterator();
  				...
  				Group group = SiteService.findGroup(ad.getAgentIdString());
  				if (group != null) {
  					...
  					if(group.getDescription()!=null)
  						grDescription = " : " + group.getDescription();
  					SelectItem item = new SelectItem(group.getId(),	group.getTitle() + grDescription);
  					groupList_selected.add(item);
  					if(!serialized_groupList.equals(""))
  						serialized_groupList += ",";
  					serialized_groupList += group.getId();
  				} else {
  					try {
  						//it's a user
  						User user = UserDirectoryService.getUser(ad.getAgentIdString());
  						userList_selected.add(new SelectItem(user.getId(), user.getSortName()));
  						if(!serialized_userList.equals(""))
  								serialized_userList += ",";
  						serialized_userList += user.getId();
  					} catch(Exception e){
  						//e.printStackTrace();
  						}
  				}
...
// dateString is in client timezone, change it to server time zone
  ...
  public SelectItem[] getPublishingTargets(){
    ...
    int numSelections = 3;
   	...
   	while (iter.hasNext()){
	    ...
	    /*else if (numSelections == 4 && t.equals(AssessmentAccessControl.RELEASE_TO_SELECTED_GROUPS)) {
	    	  target[3] = new SelectItem(t, rb.getString("selected_groups"));
	    }*/
	    else if (t.equals(AgentFacade.getCurrentSiteName())) {
	  	  	target[1] = new SelectItem(t, t);
	    }
	    else if (t.equals(AssessmentAccessControlIfc.RELEASE_TO_SELECTED_STUDENTS)) {
	    	target[2] = new SelectItem(t, rb.getString("assign_students"));
	    }
    }
    return target;
  }
  ...
  public String[] getGroupsAuthorized() {
	 if(releaseTo!=null && !releaseTo.equals(AssessmentAccessControl.RELEASE_TO_SELECTED_STUDENTS)) {//TODO para calendar?
		 return null;
	 }
  ...
  public String getGradebookItem(){
	  return this.gradebookItem;
  }
  
  public void setGradebookItem(String gradebookItem){
	  this.gradebookItem = gradebookItem;
  }
	    
  public String getConditionalOptionSelected(){
	  return this.conditionalOptionSelected;
  }
	    
  public void setConditionalOptionSelected(String conditionalOptionSelected){
	  this.conditionalOptionSelected = conditionalOptionSelected;
	  if(this.conditionalOptionSelected != null)
		  if(this.conditionalOptionSelected.startsWith("9") || this.conditionalOptionSelected.startsWith("10"))
			  this.renderConditionArgument = true;
		  else{
			  this.renderConditionArgument = false;
			  this.conditionArgument = "";//??
		  }
  }
	    
  public boolean getCheckConditionalRelease() {
	  return this.checkConditionalRelease;
  }
	  
  public void setCheckConditionalRelease(boolean checkConditionalRelease) {
	  this.checkConditionalRelease = checkConditionalRelease;
  }
  ...
  public boolean isRenderConditionArgument() {
	  return renderConditionArgument;
  }
	  
  public void setRenderConditionArgument(boolean renderConditionArgument) {
	  this.renderConditionArgument = renderConditionArgument;
  }  
  ...
  public SelectItem[] getSecureDeliverModuleSelections() {
  	...
  	public void setExistingGradebook(SelectItem[] existingGradebook) {
		this.existingGradebook = existingGradebook;
	}
