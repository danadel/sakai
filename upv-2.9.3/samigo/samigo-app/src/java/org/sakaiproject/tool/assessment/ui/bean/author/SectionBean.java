/**********************************************************************************
 * $URL: https://source.sakaiproject.org/svn/msub/upv.es/PoliformaT/upv-2.9.3/samigo/samigo-app/src/java/org/sakaiproject/tool/assessment/ui/bean/author/SectionBean.java $
 * $Id: SectionBean.java 133129 2014-01-09 15:10:24Z anueda@asic.upv.es $
 ***********************************************************************************
 *
  ...
  public ArrayList getPoolsAvailable()
  {
    ...
    for (int i=0; i<sectionList.size();i++){
      ...
      if (assessmentBean.getAssessment() != null && assessmentBean.getAssessment().getSectionSet() != null) {
      	...
      	while (iter.hasNext()) {
      		SectionDataIfc sf = (SectionDataIfc)iter.next();
      		if (sf.getSectionId().toString().equals(sectionId)) {
      			section = sf;
      			break;
      		}
      	}
      ...
      // Si no hemos cargado el examen con sus secciones, cargamos la seccion
    	  ...
    	  section= assessdelegate.getSection(s.getValue().toString());
      }
