...
public class ShowAttachmentMediaServlet extends HttpServlet
{
      ...
      throws ServletException, IOException
  {
	...
	try {
		...
		boolean permission=AssessmentService.getContentHostingService().allowGetResource(resourceId);
		String referer=req.getHeader("Referer");
		boolean force = false;
		if (!permission && referer!=null && referer.endsWith("jsf/delivery/deliverAssessment")) {
		   force=true;
		   System.out.println("UPV-ForzarAcceso:BaseContentService, resource="+resourceId+" referer="+referer);
		};	
		
		// create a copy of the resource
		if (force)  {cr = AssessmentService.getContentHostingService().findResource(resourceId); }
		      else  cr = AssessmentService.getContentHostingService().getResource(resourceId);
		if (cr == null) {
			return;
		}
		media = cr.getContent();
