...
public class RepublishAssessmentListener implements ActionListener {
	...
	public void processAction(ActionEvent ae) throws AbortProcessingException {
		...
		boolean gbUpdated = updateGB(assessment, context);
		if (!gbUpdated){
	...
	private boolean updateGB(PublishedAssessmentFacade assessment, FacesContext context) {
			...
			if (evaluation.getToGradeBook() != null	&& 
				...
				try {
					if (evaluation.getToGradeBook().equals(EvaluationModelIfc.TO_DEFAULT_GRADEBOOK.toString())) {
						/* Aqui no haría falta porque se elimina y se añade el gradebook 
					...
					for (int i = 0; i < list.size(); i++) {
						try {
							...
							if (evaluation.getToGradeBook().equals(EvaluationModelIfc.TO_DEFAULT_GRADEBOOK.toString()))
								...
								String gradebookName = assessment.getAssessmentMetaDataByLabel(AssessmentMetaDataIfc.TO_GRADEBOOK_NAME);
