/**********************************************************************************
 * $URL: https://source.sakaiproject.org/svn/msub/upv.es/PoliformaT/upv-2.9.3/samigo/samigo-app/src/java/org/sakaiproject/tool/assessment/ui/listener/select/SelectActionListener.java $
 * $Id: SelectActionListener.java 321540 2015-10-19 10:20:29Z anueda@asic.upv.es $
 ***********************************************************************************
 *
 ...
 * @author Ed Smiley
 * @version $Id: SelectActionListener.java 321540 2015-10-19 10:20:29Z anueda@asic.upv.es $
 */
...
public class SelectActionListener
  ...
  public boolean isAvailable(PublishedAssessmentFacade f, HashMap h, HashMap numberRetakeHash, HashMap actualNumberRetakeHash, List updatedAssessmentNeedResubmitList, List updatedAssessmentList) {
	...
	if(returnValue == true){
		...
		AssessmentAccessControlIfc pubControl = pubAssessmentService.loadPublishedAccessControl(f.getPublishedAssessmentId());
		if (pubControl.isUseConditionalRelease() != null && pubControl.isUseConditionalRelease().booleanValue()) {
			if(f.getSatisfiesRule()!=null){
				if(f.getSatisfiesRule()==true){
					returnValue = true;
				} else if(f.getSatisfiesRule()==false){
					returnValue = true;
				}
			} else if(f.getConditionalAccessList()!=null){
				if(f.getConditionalAccessList().contains(AgentFacade.getAgentString())){
					returnValue = true;
				} else{
					returnValue = false;
				}
