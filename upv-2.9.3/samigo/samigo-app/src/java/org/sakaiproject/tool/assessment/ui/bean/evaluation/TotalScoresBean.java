/**********************************************************************************
 * $URL: https://source.sakaiproject.org/svn/msub/upv.es/PoliformaT/upv-2.9.3/samigo/samigo-app/src/java/org/sakaiproject/tool/assessment/ui/bean/evaluation/TotalScoresBean.java $
 * $Id: TotalScoresBean.java 320028 2015-07-02 07:50:01Z anueda@asic.upv.es $
 ***********************************************************************************
 *
  ...
  implements Serializable, PhaseAware
{
  ...
  private String selectedEvaluationMode = "1";
  ...
  private String globalTotalScore = "-";
  private String globalTotalOverrideScore = "-";
 ...
 public void setSortAscending(boolean sortAscending)
 {
  ...
  public String getSelectedEvaluationMode() {
	  return selectedEvaluationMode;
  }
	  
  public void setSelectedEvaluationMode(String param ) {
	  this.selectedEvaluationMode = param;
  }
...
*/  
  ...
  private List getSectionEnrollments(String sectionid) {
    ...
    return service.getSectionEnrollments(AgentFacade.getCurrentSiteId(), sectionid , AgentFacade.getAgentString());
  	...
  	public Map getSectionEnrollments(String[] sections, String instructor) {
	    GradingSectionAwareServiceAPI service = new GradingSectionAwareServiceImpl();
	    
	    List enrollments = new ArrayList();
	    for (String sectionid : sections) {
	    	enrollments.addAll(service.getSectionEnrollments(AgentFacade.getCurrentSiteId(), sectionid, instructor));
	    }
...
*/
  ...
  public void clear(ActionEvent event) {
	...
	public ArrayList findMatchingAgents(final String pattern) {
		String query = (convertToAscii(pattern)).toLowerCase();
		query = query.replace(",","");
		boolean ini = false;
		StringBuilder aux = new StringBuilder();
		for(int i=0; i< query.length(); i++){
			if(query.charAt(i)=='"' && ini == false)
				ini = true;
			else if(query.charAt(i)=='"' && ini == true)
				ini = false;
			if(ini==true && query.charAt(i)==' ')
				aux.append('_');
			else
				aux.append(query.charAt(i));
		}
		String[] searchWords = aux.toString().split(" ");
		...
		for(Iterator iter = allAgents.iterator(); iter.hasNext();) {
			...
			name1 = result.getFirstName() + " " + result.getLastName();
			name1 = convertToAscii(name1).toLowerCase();
			name2 = result.getLastName() + " " + result.getFirstName();
			name2 = convertToAscii(name2).toLowerCase();
			String name = convertToAscii(result.getFirstName()).toLowerCase();
			String lastName = convertToAscii(result.getLastName()).toLowerCase();
			String eid = convertToAscii(result.getAgentEid()).toLowerCase();
			int i;
			for(i=0; i<searchWords.length; i++){
				...
				if(name.contains(word) || lastName.contains(word) || eid.contains(word) || name1.contains(word) || name2.contains(word)){
					continue;
				}
				else {
					break;				
	...
	public boolean isReleasedToGroups() {
		...
		*/
		return (this.getPublishedAssessment().getAssessmentAccessControl().getReleaseTo().equals(AssessmentAccessControl.RELEASE_TO_SELECTED_GROUPS)
				|| this.getPublishedAssessment().getAssessmentAccessControl().getReleaseTo().equals("Assign Students"));
	}
	...
	public void exportExcel(){
		...
		ArrayList headerList = new ArrayList<Object>();
		...
		writeDataToResponse(headerList, dataList, getDownloadFileName(), response);
	...
	private void writeDataToResponse(List headerList, List dataList, String fileName, HttpServletResponse response) {
	...
	private HSSFWorkbook getAsWorkbook(List headerList, List dataList) {
		...
		Iterator<Object> dataIter = dataList.iterator();
	...
	private String getAgentResultStatusString(AgentResults agent){
		String status = "";
				
		if (agent.getStatus()==2 && agent.getAttemptDate()!=null && (anonymous=="false" || agent.getAssessmentGradingId()!=-1)){
			return ContextUtil.getLocalizedString("org.sakaiproject.tool.assessment.bundle.EvaluationMessages","auto_scored");
		}else {
			return status;
	...
	private static final String UNICODE = 	"\u00C0\u00E0\u00C8\u00E8\u00CC\u00EC\u00D2\u00F2\u00D9\u00F9"             
		...
		+ "\u00C7\u00E7" 
		+ "\u0150\u0151\u0170\u0171";
	private static final String PLAIN_ASCII =   "AaEeIiOoUuAaEeIiOoUuYyAaEeIiOoUuYyAaOoNnAaEeIiOoUuYyAaCc";
	...
	public String getGlobalTotalScore() {
		return globalTotalScore;
	}
					
	public void setGlobalTotalScore(String globalTotalScore) {
		if (globalTotalScore!= null){
			this.globalTotalScore = globalTotalScore.replace(',', '.');
		}
		else {
			this.globalTotalScore = null;
		}
	}
					
	public String getGlobalTotalOverrideScore() {
		return globalTotalOverrideScore;
	}
					
	public void setGlobalTotalOverrideScore(String globalTotalOverrideScore) {
		if (globalTotalOverrideScore!= null){
			this.globalTotalOverrideScore = globalTotalOverrideScore.replace(',', '.');
		}
		else {
			this.globalTotalOverrideScore = null;
		}
	}
					
	public String getGlobalComments() {
		return Validator.check(globalComments, "");
	}
