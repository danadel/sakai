/**********************************************************************************
 * $URL: https://source.sakaiproject.org/svn/msub/upv.es/PoliformaT/upv-2.9.3/samigo/samigo-app/src/java/org/sakaiproject/tool/assessment/ui/listener/author/AuthorActionListener.java $
 * $Id: AuthorActionListener.java 316695 2015-01-12 12:22:16Z anueda@asic.upv.es $
 ***********************************************************************************
 *
 ...
 * @author Ed Smiley
 * @version $Id: AuthorActionListener.java 316695 2015-01-12 12:22:16Z anueda@asic.upv.es $
 */
    ...
    implements ActionListener
{
  ...
  public void prepareAssessmentsList(AuthorBean author, AssessmentService assessmentService, GradingService gradingService, PublishedAssessmentService publishedAssessmentService) {
		...
		ArrayList assessmentList = assessmentService.getBasicInfoOfAllAssessments(
		...
		while (iter.hasNext()) {
			...
			Integer number = publishedAssessmentService.getNumberPublishedBasedAssessment(assessmentFacade.getAssessmentBaseId());
			if (number > 0) {
				assessmentFacade.setPublishedForDisplay(true);
			}
			else {
				assessmentFacade.setPublishedForDisplay(false);
			}
		}
		...
		iter = publishedAssessmentList.iterator();
		while (iter.hasNext()) {
			PublishedAssessmentFacade assessmentFacade= (PublishedAssessmentFacade) iter.next();
			assessmentFacade.setRemovedForDisplay(assessmentFacade.getStatus().intValue() == AssessmentBaseIfc.DEAD_STATUS);
		}
