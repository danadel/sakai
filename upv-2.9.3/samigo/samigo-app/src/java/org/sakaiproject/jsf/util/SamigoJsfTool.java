...
/**********************************************************************************
 * $URL: https://source.sakaiproject.org/svn/msub/upv.es/PoliformaT/upv-2.9.3/samigo/samigo-app/src/java/org/sakaiproject/jsf/util/SamigoJsfTool.java $
 * $Id: SamigoJsfTool.java 131695 2013-11-18 12:06:37Z anueda@asic.upv.es $
 **********************************************************************************
 *
  ...
  public class SamigoJsfTool extends JsfTool {
	...
	+ getServletContext().getServletContextName());
	 ...
	 SectionBean bean = (SectionBean) ContextUtil.lookupBeanFromExternalServlet(
	  ...
	  && ("true").equals(toolSession.getAttribute("SENT_TO_FILEPICKER_HELPER"))){
	  	//log.info("dispatch - SamigoJsfTool");
	 AssessmentSettingsBean bean = (AssessmentSettingsBean) ContextUtil.lookupBeanFromExternalServlet(
		 ...
		 Map<String,String> values = ach.loadConditionData(toolSession, bean.getNotificationId());
		 if(bean!=null){
			if(values==null){
				bean.setGradebookItem(null);
				bean.setConditionalOptionSelected(null);
				bean.setConditionArgument(null);
				bean.setCheckConditionalRelease(false);
				bean.setRenderConditionArgument(false);
				bean.setSubmittedFunctionName(null);
			} else {
				bean.setSubmittedFunctionName(values.get("submittedFunctionName"));
				bean.setGradebookItem(values.get("gradebookItem"));
				bean.setConditionalOptionSelected(values.get("conditionalOptionSelected"));
				bean.setConditionArgument(values.get("conditionArgument"));
			}
		 } else
			log.debug("BeanSettings a null en SamigoJsfTool");
