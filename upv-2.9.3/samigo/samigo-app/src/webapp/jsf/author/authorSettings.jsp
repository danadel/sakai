...
<!--
* $Id: authorSettings.jsp 314116 2014-10-08 09:58:30Z anueda@asic.upv.es $
<%--
***********************************************************************************
*
...
-->
  <f:view>
    <html xmlns="http://www.w3.org/1999/xhtml" lang="en" xml:lang="en">
      ...
      <!-- SECTIONS -->
	  <samigo:script path="/js/jquery.picklist.js"/>
...
function showHideReleaseGroups(){
  ...
  for (i = 0; i <inputList.length; i++) 
  {
    if(inputList[i].type=='radio')
    {
      ...
      if(inputList[i].value.indexOf("Assign Students")>=0) {
    	showAdhocGroups=inputList[i].checked;
    	if(breakAux)
    		break;
    	else
    		breakAux = true;
  ...
  if(showAdhocGroups) {
	document.getElementById("groupAdhocDiv").style.display = "block";
	document.getElementById("groupAdhocDiv").style.width = "80%";
  }
  else {
	document.getElementById("groupAdhocDiv").style.display = "none";
  }
}
...
{  
   ...
   for (i=0 ; i < blockElements.length; i++)
   {
      ...
      if(divisionNo.indexOf("__hide_division_assessmentSettingsAction") >=0 && blockElements[i].style.display != "none")
      { 
         //alert("divisionNo=" + divisionNo);
   ...
   document.forms[0].elements['assessmentSettingsAction:blockDivs'].value = blockDivs;
    ...
    <body onload="reloadAllListsValues();checkTimeSelect();<%= request.getAttribute("html.body.onload") %>">
<div id="pasteApplet"></div>
...
<f:subview id="intro">
  ...
  <f:verbatim></div></f:verbatim>
   ...
   <f:verbatim><div id="groupAdhocDiv" class="tier3"></f:verbatim>
  	<h:panelGrid columns="3" style="margin-bottom:20px;" rendered="#{assessmentSettings.hasAvailableGroups}">
		<h:outputText value="#{assessmentSettingsMessages.available_groups}" style="font-weight: bold; "/>
		<h:outputText value="" styleClass="panelCell"/>
		<h:outputText value="#{assessmentSettingsMessages.selected_groups}" style="font-weight: bold; "/>
		
		<h:selectManyListbox style="width: 350px; height:200px;" id="groupList_available" ondblclick="move('assessmentSettingsAction:groupList_available','assessmentSettingsAction:groupList_selected', false);">  
			<f:selectItems  value="#{assessmentSettings.groupList_available}"/>
		</h:selectManyListbox>	
								
		<h:panelGrid columns="1" style="text-align:center;">
			<h:commandButton value=">" title="#{assessmentSettingsMessages.add_group}" onclick="move('assessmentSettingsAction:groupList_available','assessmentSettingsAction:groupList_selected', false); return false;" />
			<h:commandButton value="<" title="#{assessmentSettingsMessages.remove_group}" onclick="move('assessmentSettingsAction:groupList_selected','assessmentSettingsAction:groupList_available', false); return false;" />
			<h:commandButton value=">>" title="#{assessmentSettingsMessages.add_all_groups}" onclick="move('assessmentSettingsAction:groupList_available','assessmentSettingsAction:groupList_selected', true); return false;" />
			<h:commandButton value="<<" title="#{assessmentSettingsMessages.remove_all_groups}" onclick="move('assessmentSettingsAction:groupList_selected','assessmentSettingsAction:groupList_available', true); return false;" />
		</h:panelGrid>
		
		<h:selectManyListbox style="width: 350px; height:200px;" id="groupList_selected" ondblclick="move('assessmentSettingsAction:groupList_selected','assessmentSettingsAction:groupList_available', false);">
			<f:selectItems  value="#{assessmentSettings.groupList_selected}"/>  
		</h:selectManyListbox>
		
	</h:panelGrid>
	<h:inputHidden id="groupList_hidden" value="#{assessmentSettings.serialized_groupList}" />
    <h:panelGrid columns="3">
		<h:outputText value="#{assessmentSettingsMessages.available_users}" style="font-weight: bold; "/>
		<h:outputText value="" styleClass="panelCell"/>
		<h:outputText value="#{assessmentSettingsMessages.selected_users}" style="font-weight: bold; "/>
		
		<h:selectManyListbox style="width: 350px; height:200px;" id="userList_available" ondblclick="move('assessmentSettingsAction:userList_available','assessmentSettingsAction:userList_selected', false);">  
			<f:selectItems  value="#{assessmentSettings.userList_available}"/>  
		</h:selectManyListbox>

		<h:panelGrid columns="1" style="text-align:center;">
			<h:commandButton value=">" title="#{assessmentSettingsMessages.add_user}" onclick="move('assessmentSettingsAction:userList_available','assessmentSettingsAction:userList_selected', false); return false;" />
			<h:commandButton value="<" title="#{assessmentSettingsMessages.remove_user}" onclick="move('assessmentSettingsAction:userList_selected','assessmentSettingsAction:userList_available', false); return false;" />
			<h:commandButton value=">>" title="#{assessmentSettingsMessages.add_all_users}" onclick="move('assessmentSettingsAction:userList_available','assessmentSettingsAction:userList_selected', true); return false;" />
			<h:commandButton value="<<" title="#{assessmentSettingsMessages.remove_all_users}" onclick="move('assessmentSettingsAction:userList_selected','assessmentSettingsAction:userList_available', true); return false;" />
		</h:panelGrid>
		
		<h:selectManyListbox style="width: 350px; height:200px;" id="userList_selected" ondblclick="move('assessmentSettingsAction:userList_selected','assessmentSettingsAction:userList_available', false);">
			<f:selectItems  value="#{assessmentSettings.userList_selected}"/>  
		</h:selectManyListbox>				
   </h:panelGrid>
   <h:inputHidden id="userList_hidden" value="#{assessmentSettings.serialized_userList}" />
  ...
  <!-- *** CONDITIONAL RELEASE *** -->
<h:panelGroup rendered="#{conditionsEnabled==true}" >
<samigo:hideDivision title="#{assessmentSettingsMessages.heading_conditional_release}">
  <f:verbatim><div class="tier2"></f:verbatim>
   <h:panelGrid border="0" columns="2"
    summary="Resumen">
	<h:selectBooleanCheckbox
		 id="checkConditionalRelease"
         rendered="#{noGradebookItems==false}"
         value="#{assessmentSettings.checkConditionalRelease}"/>
	<h:outputText value="#{assessmentSettingsMessages.cr_only_if_condition_is_met}"
        rendered="#{noGradebookItems==false}"/>
	</h:panelGrid>
	<h:panelGrid border="0" columns="4" summary="Resumen" rendered="#{noGradebookItems==false}">
		<h:outputText value="#{assessmentSettingsMessages.cr_gradebook_tool_name}" rendered="#{noGradebookItems==false}"/>
		<h:selectOneMenu id="gradebookItem" value="#{assessmentSettings.gradebookItem}" disabled="false">
          <f:selectItems value="#{resourceSelectionsSelectItems}" />
		</h:selectOneMenu>
		<h:selectOneMenu id="conditionalOptionSelected" value="#{assessmentSettings.conditionalOptionSelected}" onchange="getAllListValue();setBlockDivs();document.forms[0].onsubmit();document.forms[0].submit();" disabled="false">
          <f:selectItems value="#{conditionSelectionsSelectItems}" />
		</h:selectOneMenu>
		<h:inputText id="conditionArgument" size="4" value="#{assessmentSettings.conditionArgument}" rendered="#{assessmentSettings.renderConditionArgument == true}" />
	</h:panelGrid>
	<h:panelGrid border="0" columns="4" summary="Resumen" rendered="#{noGradebookItems==true}">
		<h:outputText value="#{assessmentSettingsMessages.no_gradebook_assignments}" />
	</h:panelGrid>
   <f:verbatim></div></f:verbatim>
  </samigo:hideDivision>
</h:panelGroup>
...
<h:outputText escape="false" value="<br/>#{assessmentSettingsMessages.ip_note} <br/>#{assessmentSettingsMessages.ip_example}#{assessmentSettingsMessages.ip_ex}<br/>"/> 
	  ...
	  <h:panelGrid border="0" columns="1"  columnClasses="longtext"
		rendered="#{assessmentSettings.valueMap.lockedBrowser_isInstructorEditable==true && assessmentSettings.secureDeliveryAvailable}">
	  	<h:selectOneRadio id="secureDeliveryModule" value="#{assessmentSettings.secureDeliveryModule}"  layout="pageDirection" onclick="getAllListValue();setBlockDivs();document.forms[0].onsubmit();document.forms[0].submit();">
...
--%>
    <h:panelGrid
      ...
      <h:panelGroup>
        <h:selectBooleanCheckbox id="selTimeAssess" onclick="getAllListValue();checkUncheckTimeBox();setBlockDivs();document.forms[0].onsubmit();document.forms[0].submit();"
...
<h:panelGroup rendered="#{assessmentSettings.valueMap.itemAccessType_isInstructorEditable==true or assessmentSettings.valueMap.displayChunking_isInstructorEditable==true or assessmentSettings.valueMap.displayNumbering_isInstructorEditable==true }" >
  ...
  <f:verbatim> <div class="longtext"></f:verbatim> <h:outputLabel for="itemNavigation" value="#{assessmentSettingsMessages.navigation}" /><f:verbatim></div><div class="tier3"></f:verbatim>
      <h:panelGrid columns="1">
		...
		onclick="getAllListValue();setBlockDivs();updateItemNavigation(true);submitForm();">
    ...
    <f:verbatim></div><div class="tier3"></f:verbatim>
      ...
      <h:panelGrid border="0" columns="1"  >
        <h:selectOneRadio id="feedbackDelivery" value="#{assessmentSettings.feedbackDelivery}"
           layout="pageDirection" onclick="getAllListValue();setBlockDivs();disableAllFeedbackCheck(this.value);">
    ...
    <h:panelGroup rendered="#{assessmentSettings.valueMap.feedbackComponents_isInstructorEditable==true}">
     <f:verbatim> <div class="longtext"> </f:verbatim>  
       ...
       <h:panelGrid columns="2"  >
        ...
        onclick="getAllListValue();setBlockDivs();disableOtherFeedbackComponentOption(this);" layout="pageDirection">
...
<f:verbatim></div></f:verbatim>
     ...
     <f:verbatim>  <div class="longtext">  </f:verbatim> <h:outputLabel value="#{assessmentSettingsMessages.gradebook_options}" /><f:verbatim></div> <div class="tier3"> </f:verbatim>
      <h:panelGrid columns="2" rendered="#{assessmentSettings.firstTargetSelected != 'Anonymous Users'}">
        <h:selectOneRadio id="toDefaultGradebook1" value="#{assessmentSettings.toDefaultGradebook}"  layout="pageDirection"
        	onclick="getAllListValue();setBlockDivs();submitForm();">
      ...
      <h:panelGrid columns="2" rendered="#{assessmentSettings.firstTargetSelected != 'Anonymous Users' && assessmentSettings.toDefaultGradebook == 3}">
      	<h:selectOneMenu id="toGradebook" value="#{assessmentSettings.gradebookName}" >
   ...
   <f:verbatim>  <div class="longtext">  </f:verbatim> 
   <h:outputLabel for="scoringType" value="#{assessmentSettingsMessages.recorded_score}" rendered="#{author.canRecordAverage}"/>
   <h:outputLabel for="scoringType2" value="#{assessmentSettingsMessages.recorded_score}" rendered="#{!author.canRecordAverage}"/>
   <f:verbatim></div> <div class="tier3"> </f:verbatim>
      <h:panelGrid columns="2"  >
        <h:selectOneRadio id="scoringType" value="#{assessmentSettings.scoringType}" layout="pageDirection" rendered="#{author.canRecordAverage}">
        ...
        <h:selectOneRadio id="scoringType2" value="#{assessmentSettings.scoringType}" layout="pageDirection" rendered="#{!author.canRecordAverage}">
          <f:selectItem itemValue="1" itemLabel="#{assessmentSettingsMessages.highest_score}"/>
          <f:selectItem itemValue="2" itemLabel="#{assessmentSettingsMessages.last_score}"/>
        </h:selectOneRadio>
      </h:panelGrid>
...
--%>
 ...
 <!-- save & publish -->
  <h:commandButton  value="#{assessmentSettingsMessages.button_unique_save_and_publish}" type="submit" styleClass="active" rendered="#{assessmentSettings.hasQuestions}"
      action="#{assessmentSettings.getOutcomePublish}" onclick="getAllListValue();setBlockDivs();updateItemNavigation(false);" >
...
<!-- save -->
  <h:commandButton type="submit" value="#{assessmentSettingsMessages.button_save_settings}" action="#{assessmentSettings.getOutcomeSave}"  onclick="getAllListValue();setBlockDivs();updateItemNavigation(false);">
