...
<!--
/**********************************************************************************
 * $URL: https://source.sakaiproject.org/svn/msub/upv.es/PoliformaT/upv-2.9.3/samigo/samigo-app/src/webapp/jsf/author/publishAssessment.jsp $
 * $Id: publishAssessment.jsp 317907 2015-03-06 12:39:45Z anueda@asic.upv.es $
 ***********************************************************************************
 *
...
-->
  <f:view>
	...
	<h:outputText value="function showRepublishAlert (obj) {" escape="false" />
	<h:outputText value="if (obj.checked){" escape="false"/>
    <h:outputText value="var response = confirm('#{assessmentSettingsMessages.update_most_current_submission_checkbox_cfn}');" escape="false" /> 
    <h:outputText value="if (response) obj.checked=true;" escape="false" />
    <h:outputText value="else obj.checked=false;" escape="false" />
...
<h:panelGrid rendered="#{!author.isEditPendingAssessmentFlow && author.isRepublishAndRegrade}">
  ...
  <h:panelGroup rendered="#{publishedSettings.itemNavigation eq '2' && assessmentBean.hasSubmission}">
        <h:selectBooleanCheckbox id="updateMostCurrentSubmissionCheckbox" value="#{publishedSettings.updateMostCurrentSubmission}" onclick="javascript:showRepublishAlert(this);"/>
...
<h:panelGrid columns="5" styleClass="act" rowClasses="topAlign">
  <!-- Cancel button -->
   ...
   <h:commandButton id="publish" value="#{commonMessages.publish_action}" type="submit"
	...
	<h:commandButton  value="#{authorMessages.button_republish_and_regrade}" type="submit" styleClass="active" rendered="#{!author.isEditPendingAssessmentFlow && author.isRepublishAndRegrade}" action="#{author.getOutcome}">
		<f:actionListener type="org.sakaiproject.tool.assessment.ui.listener.author.RepublishAssessmentListener" />
	</h:commandButton>

	<h:commandButton  value="#{authorMessages.button_republish}" type="submit" styleClass="active" rendered="#{!author.isEditPendingAssessmentFlow && !author.isRepublishAndRegrade}" action="#{author.getOutcome}">
		<f:actionListener type="org.sakaiproject.tool.assessment.ui.listener.author.RepublishAssessmentListener" />
	</h:commandButton>
...
<h:panelGrid columns="1" rowClasses="shorttextPadding" rendered="#{author.isEditPendingAssessmentFlow}" border="0">
	<h:panelGroup>
		...
		<h:outputFormat value=" #{assessmentSettingsMessages.available_class_at}" escape="false" rendered="#{assessmentSettings.releaseTo ne 'Anonymous Users' && assessmentSettings.releaseTo ne 'Selected Groups' && assessmentSettings.releaseTo ne 'Assign Students'}">
			<f:param value="#{assessmentSettings.startDateString}" />
			<f:param value="#{assessmentSettings.publishedUrl}" />
		</h:outputFormat>
	...
	<h:outputFormat value=" #{assessmentSettingsMessages.available_group_ah_at}" escape="false" rendered="#{assessmentSettings.releaseTo eq 'Assign Students'}">
			<f:param value="#{assessmentSettings.startDateString}" />
			<f:param value="#{assessmentSettings.publishedUrl}" />
	</h:outputFormat>
	...
	<h:panelGrid columns="1" rowClasses="shorttextPadding" rendered="#{!author.isEditPendingAssessmentFlow}" border="0">
		<h:panelGroup>
			...
			<h:outputFormat value=" #{assessmentSettingsMessages.available_class_at}" escape="false" rendered="#{publishedSettings.releaseTo ne 'Anonymous Users' && publishedSettings.releaseTo ne 'Selected Groups' && publishedSettings.releaseTo ne 'Assign Students'}">
				<f:param value="#{publishedSettings.startDateString}" />
				<f:param value="#{publishedSettings.publishedUrl}" />
			</h:outputFormat>
		...
		<h:outputFormat value=" #{assessmentSettingsMessages.available_group_ah_at}" escape="false" rendered="#{publishedSettings.releaseTo eq 'Assign Students'}">
				<f:param value="#{publishedSettings.startDateString}" />
				<f:param value="#{publishedSettings.publishedUrl}" />
		</h:outputFormat>
