...
<!-- $Id: editPool.jsp 131738 2013-11-19 11:15:20Z anueda@asic.upv.es $
<%--
***********************************************************************************
*
...
<h:outputText  rendered="#{questionpool.importToAuthoring != 'true' && questionpool.canAddQuestions && questionpool.canPreviewQuestions}" value=" #{questionPoolMessages.separator} " />
<h:commandLink title="#{questionPoolMessages.preview}" rendered="#{questionpool.importToAuthoring != 'true' && questionpool.canPreviewQuestions}"  id="previewlink" immediate="true" action="#{questionpool.startPreviewPool}">
...
<h:panelGroup rendered ="#{questionpool.currentPool.numberOfQuestions >= 1}">
  <h:commandButton id="removeSubmit"   rendered="#{questionpool.importToAuthoring == 'false' && questionpool.canDeleteQuestions}"  action="#{questionpool.doit}" value="#{commonMessages.remove_action}">
    <f:actionListener type="org.sakaiproject.tool.assessment.ui.listener.questionpool.StartRemoveItemsListener" />
  </h:commandButton>
 
 <h:outputText escape="false" value=" #{questionPoolMessages.separator} " rendered="#{questionpool.importToAuthoring != 'true' && questionpool.canDeleteQuestions && questionpool.canCopyQuestions}" />
 
 <h:commandButton title="#{questionPoolMessages.t_copyQuestion}" rendered="#{questionpool.importToAuthoring != 'true' && questionpool.canCopyQuestions}" id="copySubmit" immediate="true" action="#{questionpool.startCopyQuestions}" value="#{questionPoolMessages.copy}">
 
 </h:commandButton>
 
 <h:outputText escape="false" value=" #{questionPoolMessages.separator} " rendered="#{questionpool.importToAuthoring != 'true' && questionpool.canMoveQuestions && questionpool.canCopyQuestions}" />
 
 <h:commandButton title="#{questionPoolMessages.t_moveQuestion}" rendered="#{questionpool.importToAuthoring != 'true' && questionpool.canMoveQuestions}" id="moveSubmit" immediate="true" action="#{questionpool.startMoveQuestions}" value="#{questionPoolMessages.move}">
 </h:commandButton>
 </h:panelGroup>
 </div>
