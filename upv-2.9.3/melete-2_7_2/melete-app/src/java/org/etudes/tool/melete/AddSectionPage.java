/**********************************************************************************
 *
 * $URL: https://source.sakaiproject.org/contrib/etudes/melete/tags/2.7.2/melete-app/src/java/org/etudes/tool/melete/AddSectionPage.java $
    ...
    */
    public String getContentEditor()
    {
		ResourceLoader bundle = new ResourceLoader("org.etudes.tool.melete.bundle.Messages");
          ...
          {
                  this.contentEditor = new String(bundle.getString("compose_content"));
          }
...
//			Step2: if section has content then only create section resource and resource
			...
			//	Step 2.2: add the new resource to course site module /uploads collection
				// in case of upload and link, resource is added on clicking Continue
					...
					{
						contentEditor = convetirContenido(contentEditor);
	  	...
	  	Integer selectedSectionIdx = (Integer)sessionMap.get("selectedSectionIdx");
	  	if (selectedSectionIdx != null) {
	  		sessionMap.remove("selectedSectionIdx");
	  		module = moduleService.getModule(module.getModuleId()); //reload the module, bears in mind the created section previously
	  		
	  		// get the selected section
	  		String seqXml = module.getSeqXml();
	  		int index = 0, count = 0;
	  		String sectionId = "";
	  		while (index + 12 < seqXml.length() && count++ <= selectedSectionIdx) {
	  			index = seqXml.indexOf("section id=", index + 12);
	  		}
	  		sectionId = seqXml.substring(index + 12, seqXml.indexOf("\"", index + 12));
	  		
	  		SectionObjService sectionObj =  sectionService.getSection(Integer.parseInt(sectionId));
		  	
			try {
				moduleService.doChangeSectionLevel(module, sectionObj, section);
			} catch (MeleteException e) {
				logger.error("Error on changing the order of module. " + e.getMessage());
			}
	  	}
	  	
		//Track the event
		EventTrackingService.post(EventTrackingService.newEvent("melete.section.new", ToolManager.getCurrentPlacement().getContext() + "/sectionId=" + section.getSectionId(), true));
    ...
    private String convetirContenido(String sferyx) {
    	FacesContext context = FacesContext.getCurrentInstance();
		Map sessionMap = context.getExternalContext().getSessionMap();
		MeleteSiteAndUserInfo siteInfo = (MeleteSiteAndUserInfo)sessionMap.get("meleteSiteAndUserInfo");
		
		String serverUrl = siteInfo.getMeleteDocsLocation();
				
    	String sferyx2 = "";

    	// Buscamos imagenes locales ...
    	int pos = 0;
    	while (sferyx.indexOf("<img", pos) != -1 || sferyx.indexOf("<a href", pos) != -1) {
    		
    		int posImg = sferyx.indexOf("<img", pos);
    		int posHref = sferyx.indexOf("<a href", pos);
    		
    		int index1;
    		int posComillas;
    		if ((posImg == -1) || (posImg != -1 && posHref != -1 && posHref < posImg)) {
    			index1 = sferyx.indexOf("<a href=", pos);
    			posComillas = 9;
    		}
    		else {
    			index1 = sferyx.indexOf("src=", pos);
    			posComillas = 5;
    		}
    		// Transformamos de dir.local de la imagen a dir.servidor de la imagen
    		int index2 = sferyx.indexOf("\"", index1+posComillas);
    		
    		// campo 'src' de la imagen
    		String src = sferyx.substring(index1+posComillas, index2);
    		String[] array = src.split("/");
    		String nombre = array[array.length-1];
    		
    		String sferyxAux = sferyx.substring(pos, index2);
    		
    		// Solamente cambia la url de local a servidor si el fichero es de sferyx
    		if (src.indexOf("file:", 0) != -1)
    			// Formamos la url a la imagen en el Content Manager
    			src = serverUrl + nombre;
    		
    		// Contiene la url que introduciremos en el editor FCK
    		sferyx2 = sferyx2 + sferyxAux.substring(0,index1+posComillas-pos) + src + "\"";
				
    		pos = index2+1;
    	}
    	
    	if (sferyx.length() > pos) 
			sferyx2 = sferyx2 + sferyx.substring(pos);
    	
    	return sferyx2;
    }
    
    private int getLevelNode(org.w3c.dom.Element element) {
    	int level = 0;
    	org.w3c.dom.Node parent = element.getParentNode();
		while (parent != null && "section".equals(parent.getNodeName())) {
