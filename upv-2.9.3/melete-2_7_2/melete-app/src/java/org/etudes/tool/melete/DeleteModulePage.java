/**********************************************************************************
 *
 * $URL: https://source.sakaiproject.org/contrib/etudes/melete/tags/2.7.2/melete-app/src/java/org/etudes/tool/melete/DeleteModulePage.java $
		...
		try{
			...
			if (getModuleSelected() == true)
			{
				...
				while (it.hasNext()){
					  ...
					  //Track the event
					  EventTrackingService.post(EventTrackingService.newEvent("melete.module.delete", ToolManager.getCurrentPlacement().getContext() + "/moduleId=" + obj.getModuleId(), true));
					  if(obj.getNotificationId()!=null){
						MeleteConditionsHelper mch = new MeleteConditionsHelper();
						mch.removeExistingNotification(obj.getNotificationId());
					  }
				}
			...
			if (getSectionSelected() == true)
			{
				...
				while (it.hasNext()){
					  ...
					  //Track the event
					  EventTrackingService.post(EventTrackingService.newEvent("melete.section.delete", ToolManager.getCurrentPlacement().getContext() + "/sectionId=" + section.getSectionId(), true));
				}
