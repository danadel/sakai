/**********************************************************************************
 *
 * $URL: https://source.sakaiproject.org/contrib/etudes/melete/tags/2.7.2/melete-app/src/java/org/etudes/tool/melete/ViewSectionsPage.java $
...
public class ViewSectionsPage implements Serializable/*,ToolBean */{
      ...
      private List<SectionObjService> sections = new ArrayList<SectionObjService>();
  ...
  /**
    ...
    public ModuleObjService getModule()
    {
    	...
    	try {
  	  	  ...
  	  	  this.nextSeqNo = getNextModuleCR(this.moduleSeqNo);
  	  	  ...
  	  	  SectionObjService thisSection = (SectionObjService) getSectionService().getSection(this.sectionId);
  	  	  Iterator iter = module.getSections().values().iterator();
  	  	  sections.clear();
  	  	  	  	  
  	  	  String[] sdst = getSectionService().getSectionDisplaySequence(thisSection).split("\\.");
  	  	  	  	
  	  	  while (iter.hasNext()) {
  	  		  SectionObjService section = (SectionObjService)iter.next();
  	  		  section.setSectionDisplaySequence(getSectionService().getSectionDisplaySequence(section));
  	  		  	  		
  	  		  String[] sdso = section.getSectionDisplaySequence().split("\\.");
  	  		  if (sdst.length >= sdso.length) {
  	  			  boolean equals = true;
  	  			  for (int i=0; i<sdso.length; i++) {
  	  				  if (!sdst[i].equals(sdso[i])) {
  	  					  equals = false;
  	  					  break;
  	  				  }
  	  			  }
  	  			  if (equals) this.sections.add(section);
  	  		  }
  	  	  }
...
public void setMeleteCHService(MeleteCHService meleteCHService) {
     ...
     * @return the sections
     */
    public List<SectionObjService> getSections() {
    	return sections;
    }
     ...
     */
    public void setSections(List<SectionObjService> sections) {
    	this.sections = sections;
    }
    
	private int getNextModuleCR(int moduleSeqNo){
		int auxNextSeqNo = getModuleService().getNextSeqNo(getCourseId(), moduleSeqNo, getInstRole());
		Module modd = null;
		if(auxNextSeqNo > 0){//si tiene nextSeqNo
			modd = (Module) getModuleService().getModuleBySeq(courseId, auxNextSeqNo);
		} else
			return auxNextSeqNo;
		...
		if(!shouldAdd){
			return getNextModuleCR(auxNextSeqNo);
		} else	
			return auxNextSeqNo;
	}
