/**********************************************************************************
 *
 * $URL: https://source.sakaiproject.org/contrib/etudes/melete/tags/2.7.2/melete-app/src/java/org/etudes/tool/melete/MeleteSiteAndUserInfo.java $
...
public class MeleteSiteAndUserInfo {
	...
	*******************************************************************************/
	...
	public boolean isSafariIpad() {
		FacesContext context = FacesContext.getCurrentInstance();
		String agent = ((HttpServletRequest)context.getExternalContext().getRequest()).getHeader("User-Agent");
		
		if (agent.toLowerCase().contains("safari") && agent.toLowerCase().contains("mobile")) {
			return true;
		}
		return false;
	}
