/**********************************************************************************
 *
 * $URL: https://source.sakaiproject.org/contrib/etudes/melete/tags/2.7.2/melete-app/src/java/org/etudes/tool/melete/ListAuthModulesPage.java $
 *
 ***********************************************************************************
...
public class ListAuthModulesPage implements Serializable
{
	...
	public String editAction()
	{
		...
		if (moduleSelected)
		{
			...
			//CR
			emPage.setNotificationId(FormattedText.convertFormattedTextToPlaintext(mdbean.getModule().getNotificationId()));
			ToolSession toolSession = SessionManager.getCurrentToolSession();
			MeleteConditionsHelper mch = new MeleteConditionsHelper();
			Map<String,String> values = mch.loadConditionData(toolSession, emPage.getNotificationId());
			if(emPage!=null){
				if(values==null){
					emPage.setGradebookItem(null);
					emPage.setConditionalOptionSelected(null);
					emPage.setConditionArgument(null);
					emPage.setCheckConditionalRelease(false);
					emPage.setRenderConditionArgument(false);
					emPage.setSubmittedFunctionName(null);
				} else {
					emPage.setSubmittedFunctionName(values.get("submittedFunctionName"));
					emPage.setGradebookItem(values.get("gradebookItem"));
					emPage.setConditionalOptionSelected(values.get("conditionalOptionSelected"));
					emPage.setConditionArgument(values.get("conditionArgument"));
					emPage.setCheckConditionalRelease(true);
				}
			}
	...
	public String AddModuleAction()
	{
		...
		ToolSession toolSession = SessionManager.getCurrentToolSession();
		MeleteConditionsHelper mch = new MeleteConditionsHelper();
		Map<String,String> values = mch.loadConditionData(toolSession, null);
		if(amPage!=null){
			if(values==null){
				amPage.setGradebookItem(null);
				amPage.setConditionalOptionSelected(null);
				amPage.setConditionArgument(null);
				amPage.setCheckConditionalRelease(false);
				amPage.setRenderConditionArgument(false);
				amPage.setSubmittedFunctionName(null);
			} else {
				amPage.setSubmittedFunctionName(values.get("submittedFunctionName"));
				amPage.setGradebookItem(values.get("gradebookItem"));
				amPage.setConditionalOptionSelected(values.get("conditionalOptionSelected"));
				amPage.setConditionArgument(values.get("conditionArgument"));
			}
		}
	...
	public String AddContentAction()
	{
		...
		if (moduleSelected || sectionSelected)
		{
			...
			if (sectionSelected)
				sessionMap.put("selectedSectionIdx", new Integer(selectedSecIndex));
			count = 0;
			moduleSelected = false;
			// Mallika -3/24/05
			sectionSelected = false;
