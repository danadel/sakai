/**********************************************************************************
 *
 * $URL: https://source.sakaiproject.org/contrib/etudes/melete/tags/2.7.2/melete-app/src/java/org/etudes/tool/melete/ViewModulesPage.java $
...
//    This needs to be set later using Utils.getBinding
      ...
      if (this.mdbean == null)
 	  {
    	...
    	String userId = getUserId();
    	 if (this.moduleId > 0)
		  ...
		  this.prevSeqNo = getPrevModuleCR(this.moduleSeqNo);
	...
	private int getPrevModuleCR(int moduleSeqNo){
		int auxPrevSeqNo = getModuleService().getPrevSeqNo(getCourseId(), moduleSeqNo, getInstRole());
		Module modd = null;
		if(auxPrevSeqNo > 0){//si tiene prevSeqNo
			modd = (Module) getModuleService().getModuleBySeq(courseId, auxPrevSeqNo);
		} else
			return auxPrevSeqNo;
		boolean shouldAdd = true;
		if(modd!=null){
		...
		if(!shouldAdd){
			return getPrevModuleCR(auxPrevSeqNo);
		} else	
			return auxPrevSeqNo;
	}
}
