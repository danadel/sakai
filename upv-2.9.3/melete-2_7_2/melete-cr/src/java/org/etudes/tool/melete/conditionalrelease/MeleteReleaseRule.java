...
public class MeleteReleaseRule implements Rule, Obsoletable {
	...
	private static Log log = LogFactory.getLog(MeleteReleaseRule.class);
	...
	private ConditionService conditionService = (ConditionService) ComponentManager.get("MeleteConditionService");
	...
	private ModuleService moduleService = (ModuleService)ComponentManager.get("org.etudes.api.app.melete.ModuleService");
	...
	public MeleteReleaseRule() {

	}
	
	public MeleteReleaseRule(String resourceId, List<Condition> predicates, Conjunction conj) {
	...
	public void notify(Notification notification, Event event) {
		...
		if(notification.getProperties().getProperty("AssociatedTool")!=null && notification.getProperties().getProperty("AssociatedTool").equals("sakai.melete")){
			
			String siteId = "/site/" + notification.getProperties().getProperty("AssociatedSite");
			
			String moduleId = this.resourceId;
			
			ModuleObjService mod = moduleService.getModule(Integer.parseInt(this.resourceId));
			if(mod == null) return;//para las notificaciones que se queden perdidas
			if (("gradebook.updateItemScore").equals(event.getEvent())) {
				...
				try {
					...
					mod.setSatisfiesRule(null);
					List<String> prop = null;
					if(mod.getConditionalAccessList()!=null)
						prop = Arrays.asList( mod.getConditionalAccessList().replaceAll("\\[|\\]| ","").split(",") );
					...
					if ((shouldBeAvailable && acl.contains(grading.getUserId()))
						...
						moduleService.updateModule(mod);
					...
					if (!shouldBeAvailable && acl.contains(grading.getUserId())) {
						...
						mod.setConditionalAccessList((new ArrayList(acl)).toString());
				...
				try {
					mod.setSatisfiesRule(Boolean.valueOf(shouldBeAvailable));
					moduleService.updateModule(mod);					
				} catch(Exception e){
					e.printStackTrace();
				} finally {
	...
	public void set(NotificationAction other) {
		MeleteReleaseRule eOther = (MeleteReleaseRule) other;
