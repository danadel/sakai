/**********************************************************************************
 *
 * $URL: https://source.sakaiproject.org/contrib/etudes/melete/tags/2.7.2/melete-impl/src/java/org/etudes/component/app/melete/SubSectionUtilImpl.java $
...
public class SubSectionUtilImpl {
	...
	public String changeSectionLevel(String sectionsSeqXML, String secSelectedId, String secCreatedId) throws MeleteException
	{
		try {
			org.w3c.dom.Document document = Xml.readDocumentFromString(sectionsSeqXML);
			org.w3c.dom.Element sectionSelected = document.getElementById(secSelectedId);
			org.w3c.dom.Element sectionCreated = document.getElementById(secCreatedId);
			
			org.w3c.dom.Node sectSelectedParent = sectionSelected.getParentNode();
			org.w3c.dom.Node sectCreatedParent = sectionCreated.getParentNode();
			
			org.w3c.dom.Node newNode = sectionCreated.cloneNode(true);
			sectSelectedParent.insertBefore(newNode, sectionSelected);
			sectCreatedParent.removeChild(sectionCreated);
				
			return writeDocumentToString(document);
		}
		catch (Exception ex)
		{
			logger.error("Error on changing level in the DOM. " + ex.getMessage());
