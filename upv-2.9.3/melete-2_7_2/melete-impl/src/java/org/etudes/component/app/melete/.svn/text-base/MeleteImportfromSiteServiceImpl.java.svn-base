/**********************************************************************************
 *
 * $URL: https://source.sakaiproject.org/contrib/etudes/melete/tags/2.7.2/melete-impl/src/java/org/etudes/component/app/melete/MeleteImportfromSiteServiceImpl.java $
...
public class MeleteImportfromSiteServiceImpl extends MeleteImportBaseImpl implements MeleteImportfromSiteService{
	...
	private String processEmbedDatafromHTML(String ref_id,String fileResourceName, String toSiteId,String uploadCollId) throws Exception
	{
		...
		String charset = meleteUtil.getCharset(cr1.getContent());
		String contentEditor;
		try {
			contentEditor = new String(cr1.getContent(), charset);
		} catch (UnsupportedEncodingException e) {
			contentEditor = new String(cr1.getContent());
		}
		...
		byte[] contentEditorBytes;
		try {
			contentEditorBytes = contentEditor.getBytes(charset);
		} catch (UnsupportedEncodingException e) {
			contentEditorBytes = contentEditor.getBytes();
		}
		return getMeleteCHService().addResourceItem(fileResourceName, cr1.getContentType(),uploadCollId,contentEditorBytes,res );
	}
	...
	private void buildModules(String fromContext, String toContext)
	{
		...
		for (ListIterator i = fromModuleList.listIterator(); i.hasNext(); )
		{
			...
			Module toMod = new Module(fromMod.getTitle(), fromMod.getLearnObj(), fromMod.getDescription(), fromMod.getKeywords(), fromMod.getCreatedByFname(), fromMod.getCreatedByLname(), fromMod.getUserId(), null, null, fromMod.getInstitute(), fromMod.getWhatsNext(), fromMod.getCreationDate(), null, null);
	...
	private void transferManageItems(String fromContext, String toContext)
	{
		...
		List fromContextList = meleteCHService.getMemberNamesCollection(fromUploadsColl, fromContext);

		List toContextList = meleteCHService.getMemberNamesCollection(toUploadsColl, toContext);
