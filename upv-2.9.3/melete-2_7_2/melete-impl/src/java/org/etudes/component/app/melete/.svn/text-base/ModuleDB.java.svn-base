/**********************************************************************************
 *
 * $URL: https://source.sakaiproject.org/contrib/etudes/melete/tags/2.7.2/melete-impl/src/java/org/etudes/component/app/melete/ModuleDB.java $
...
public class ModuleDB implements Serializable {
	...
	public Module getModuleBySeq(String courseId,  int seqNo) throws HibernateException {
		 	...
		 	try
			{
		      ...
		      while (i.hasNext()) {
				    ...
				    mod = (Module) cmod.getModule();//deberia ser 1 solamente
		      }
			}
		    catch (Exception he)
		    {
...
//		    			Associate vsBeans to vmBean
		    			...
		    			if ((prevModId == 0)||(moduleId != prevModId))
		    			{
							...
							Module modd = getModule(moduleId);
							if(modd!=null){
								if(modd.isSatisfiesRule()!=null){
									if(modd.isSatisfiesRule()==true){
										shouldAdd = true;
									} else if(modd.isSatisfiesRule()==false){
										shouldAdd = true;
									}
								} else if(modd.getConditionalAccessList()!=null){
									User user = UserDirectoryService.getCurrentUser();
									if (user ==  null || user.getId() == null || ("").equals(user.getId()))//usuarios anonimos?
										shouldAdd = false;
									else {
										if(modd.getConditionalAccessList().contains(user.getId())){
											shouldAdd = true;
										} else{
											shouldAdd = false;
										}
...
// clean up code for admin tool
	...
	public void updateUrlImagesOfAllCourses() throws Exception {
		if (!meleteSecurityService.isSuperUser(UserDirectoryService.getCurrentUser().getId())) throw new MeleteException("admin_allow_cleanup");

		logger.info("Proceso de actualización de las urls de images de sites empezado");
		...
		try
		{
			...
			try
			{
				// Obtenemos la lista de cursos de la tabla MELETE_COURSE_MODULE
				String queryString = "select distinct(cmod.courseId) as courseId from CourseModule cmod";
				Query query = session.createQuery(queryString);
				List<String> res = query.list();

				logger.info("Total número de cursos: " + res.size());
				int count = 1;
				for (Iterator itr = res.listIterator(); itr.hasNext();count++)
				{
					String site = (String)itr.next();
					String[] siteSplit = site.split("_");
					
					logger.info("Procesando curso: " + site + " (" + count + "/" + res.size() + ")");
					
					// Solo limpiamos imágenes de estos tipos de sites
					if (!("ALU".equals(siteSplit[0]) || "DOC".equals(siteSplit[0]) || "GRA".equals(siteSplit[0]) ||
							"OCW".equals(siteSplit[0]))) 
						continue;
					
					List<Module> modules = getModules(site);
					
					for (Module module : modules) {
						
						Iterator iter = module.getSections().values().iterator();
						while (iter.hasNext()) {
							Section section = (Section)iter.next();
							SectionResourceService srs = section.getSectionResource();
							if (srs == null) continue;
							
							MeleteResourceService resource = srs.getResource();
							if (resource == null) continue;
							
							ContentResource cr = meleteCHService.getResource(resource.getResourceId());
							if (cr == null) continue;
							
							String content = new String(cr.getContent());
							boolean save = false;
							
							if (!content.contains("<img")) continue;
							
							Pattern p = Pattern.compile("<img[^>]*src=\"([^\"]*)" + siteSplit[0] + "_(\\d){1,5}_(\\d){1,5}" + "([^\"]*)", Pattern.DOTALL);
							...
							while (m.find()) {
								...
								if ("CFP".equals(siteSplit[0])) {
									index = img.indexOf(siteSplit[0] + "_");
								}
								else {
									index = img.indexOf(siteSplit[0] + "_" + siteSplit[1] + "_");
								}
																
								if (index > -1) {
									String siteVieja = img.substring(index, index + site.length());
									
									String img1 = img.replaceAll(siteVieja, site);
									if (!img.equals(img1)) {
										content = content.replaceFirst(img, img1);
										save = true;
									}
								}
							}
							...
							if (save) {
								meleteCHService.editResource(resource.getResourceId(), content);
								logger.info("Actualizando recurso: " + resource.getResourceId() + " en la sección: " + section.getSectionId().toString());
							}
						}
					}
				...
				long totalend = System.currentTimeMillis();
				logger.info("Finaliza el proceso de actualización de urls de imagenes de sites en: " + (totalend - totalStart) + " ms");
			}
			catch (HibernateException he)
			{
				logger.error(he.toString());
	...
	public void updatePolitubeAllCourses() throws Exception {
		if (!meleteSecurityService.isSuperUser(UserDirectoryService.getCurrentUser().getId())) throw new MeleteException("admin_allow_cleanup");

		logger.info("Proceso de actualización de videos de politube en todos los sites empezado");
		...
		try
		{
			...
			try
			{
				...
				for (Iterator itr = res.listIterator(); itr.hasNext();count++)
				{
					...
					for (Module module : modules) {
						...
						while (iter.hasNext()) {
							...
							MeleteResourceService resource;
							String content;
							try {
								SectionResourceService srs = section.getSectionResource();
								if (srs == null) continue;
								
								resource = srs.getResource();
								if (resource == null) continue;
								
								ContentResource cr = meleteCHService.getResource(resource.getResourceId());
								if (cr == null) continue;
								content = new String(cr.getContent());
								
							} catch (Exception e) {
								logger.info("Ha dado una excepcion al leer la seccion: " + e.getMessage());
								continue;
							}
							
							// Primero echamos un vistazo rapido. Si no contiene object no hay nada que hacer
							if (!content.contains("<object")) continue;
							if (!content.contains("\"videoPlayer\"")) continue;
							
							boolean save = false;
							
							Pattern pObject = Pattern.compile("<object[^>]*(name=\"videoPlayer\").*(param).*(param).*(param).*(param)[^<]*", Pattern.DOTALL);
							Matcher m = pObject.matcher(content);
							while (m.find()) {
								String politube = m.group();
								
								Pattern pParam = Pattern.compile("<param[^>]*name=\"flashvars\"[^>]*", Pattern.DOTALL);
								Matcher m2 = pParam.matcher(politube);
								
								while (m2.find()) {
									String flashvars = m2.group();
																		
									String flashvarsCorregido = flashvars.replaceAll("\"", "'");
									flashvarsCorregido = flashvarsCorregido.replace("name='flashvars'", "name=\"flashvars\"");
									flashvarsCorregido = flashvarsCorregido.replaceFirst("}'", "}\"");
									flashvarsCorregido = flashvarsCorregido.replaceFirst("value='", "value=\"");
									
									if (!flashvars.equals(flashvarsCorregido)) {
										content = content.replace(flashvars, flashvarsCorregido);
										save = true;
									}
								}
							}
				...
				long totalend = System.currentTimeMillis();
				logger.info("Finaliza el proceso de actualización de videos de politubes en: " + (totalend - totalStart) + " ms");
			}
			catch (HibernateException he)
			{
				logger.error(he.toString());
...
// end clean up deleted stuff code
	...
	public void doChangeSectionLevel(ModuleObjService module, SectionObjService secSelected, SectionObjService secCreated) throws MeleteException {
		try {
			Session session = hibernateUtil.currentSession();
			Transaction tx = null;
			try {
				SubSectionUtilImpl util = new SubSectionUtilImpl();
				
				String sectionsSeqXML = util.changeSectionLevel(module.getSeqXml(), secSelected.getSectionId().toString(), secCreated.getSectionId().toString());
				...
				if (logger.isDebugEnabled()) logger.debug("commiting transaction on changing the level of section id " + secSelected.getSectionId().toString());
			}
			catch(StaleObjectStateException sose)
			{
				logger.error("stale object exception" + sose.toString());
			...
			logger.error("changeSectionLevel_fail: " + ex.getMessage());
			throw new MeleteException(ex.getMessage());
		}
	}
